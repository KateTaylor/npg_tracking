...
sub _request_approval {
  ...
  my $instrument = $self->instrument();
  my $cis_desc = $instrument->current_instrument_status->instrument_status_dict
    ->description();
...
q{SELECT i.name, i.id_instrument, i_s.date AS date, isd.description AS description
  ...
  my $start_end = $dbh->selectall_arrayref(
...
sub convert_to_datetime_objects {
  ...
  my ( $year, $month, $day, $hour, $min, $sec ) =
    $temp_hash->{up} =~ /(\d{4})-(\d{2})-(\d{2})[ ](\d{2}):(\d{2}):(\d{2})/xms;
  ...
  ( $year, $month, $day, $hour, $min, $sec ) = $temp_hash->{down} =~
