...
sub results {
  ...
  for my $q ( @{$queries} ) {
    ...
    #warn join(q[ ], @{$q})."\n";
    push @{$results},
        @{ $util->dbh->selectall_arrayref( $q->[0], {}, $q->[1] ) };
  }
...
sub select_tags {
  ...
  my $query
      = qq(SELECT tr.id_run, t.tag FROM tag t, tag_run tr WHERE t.id_tag = tr.id_tag AND tr.id_run in ($id_runs_for_query) ORDER BY tr.id_run);
  my $dbh = $self->util->dbh();
  my $sth = $dbh->prepare(qq{$query});
  $sth->execute();
  my %run_tags_by_run;
...
sub selects {
  ...
  foreach my $for ( sort keys %{$search_for} ) {
    ...
    if ($wanted) {
      ...
      if ( $search_for->{$for}->{where} ) {
        push @{$where}, $search_for->{$for}->{where};
      }
...
sub conditions {
  ...
  foreach my $search_condition ( sort keys %{$search_conditions} ) {
    ...
    if ($condition) {
      ...
      $search_conditions->{$search_condition}->{where}
          =~ s/[?]/$condition/xms;
      push @{$where}, $search_conditions->{$search_condition}->{where};
      foreach
          my $from ( @{ $search_conditions->{$search_condition}->{from} } )
      {
        if ( !$from_seen->{$from} ) {
          push @{$from_array}, $from;
          $from_seen->{$from}++;
...
sub date_conditions {
  ...
  if ( $from || $to ) {
    if ( !$from_seen->{run_status} ) {
      push @{$from_array}, 'run_status', $from_seen->{run_status}++;
    }

    if ( !$from_seen->{run_status_dict} ) {
      push @{$from_array}, 'run_status_dict', $from_seen->{run_status_dict}++;
    }
  }
  return;
}
...
sub join_conditions {
  ...
  foreach my $table ( sort keys %{$foreign_keys} ) {
    foreach my $pair (@from_pairs) {
      if (
        ( List::MoreUtils::any { $_ eq $pair->[0] }
          @{ $foreign_keys->{$table} }
        )
        && ( List::MoreUtils::any { $_ eq $pair->[1] }
          @{ $foreign_keys->{$table} } )
          )
      {
        my $temp
...
sub search_conditions {
  ...
  return {
    ...
    is_good => {
      where => 'run_lane.is_good = 1',
      from  => ['run_lane']
    },
    loader => {
      where => q{run_status_dict.description = 'run pending'
                                    AND user.username = '?'},
      from => [ 'run_status_dict', 'run_status', 'user' ]
    },
    paired => {
      where => 'run.is_paired = 1',
      from  => ['run']
    },
    non_paired => {
      where => 'run.is_paired != 1',
    ...
    dev => {
      where => q{run.team = 'RAD'},
      from  => ['run']
    },
    instrument => {
      where => q{instrument.name = '?'},
      from  => ['instrument']
    },
    annotation => {
      where => q{annotation.comment like '%?%'},
      from  => ['annotation']
    },
    status => {
      where => q{run_status_dict.description = '?'},
      from  => [ 'run_status_dict', 'run_status' ]
    },
    libraryname => {
      where =>
          q{run.id_run in (SELECT id_run FROM st_cache WHERE type = 'library' AND content like '%?%')},
      from => ['run']
    },
    projectname => {
      where =>
          q{run.id_run in (SELECT id_run FROM st_cache WHERE type = 'project' AND content like '%?%')},
      from => ['run']
    },
    tags => {
      where => q{tag.tag in (?)},
...
sub search_for {
  ...
  return {
    projects => {
      select => 'st_cache.content AS project',
      from   => ['st_cache'],
      where  => q{type = 'project'}
    },
    annotations => {
      select => 'annotation.id_annotation, annotation.comment',
      from   => ['annotation']
    },
    batches     => { select => 'run.batch_id', from => ['run'] },
    instruments => {
      select => 'instrument.id_instrument, instrument.name',
      from   => ['instrument']
    },
    run_lanes => { select => 'run_lane.position', from => ['run_lane'] },
    status_date =>
        { select => 'DATE(run_status.date) AS date', from => ['run_status'] },
    run_status => {
      select => 'run_status_dict.description AS run_status',
