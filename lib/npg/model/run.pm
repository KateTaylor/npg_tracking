...
sub scs28 {
  ...
  if ( !exists $self->{scs28} ) {
    ...
    foreach my $mod ( @{$instrument_mods} ) {
      ...
      my $scs_date_added_strptime
          = $strptime->parse_datetime($scs_date_added);

      if ( $loading_date_strptime
        && $scs_date_added
        && DateTime->compare( $loading_date_strptime,
          $scs_date_added_strptime ) < 0 )
      {
        next;
      }
...
sub run_folder {
  ...
  if ( $model eq 'HiSeq' ) {
    ...
    if ( $self->flowcell_id() ) {
      $runfolder .= q(_A_) . $self->flowcell_id();
    }
  }
  return $runfolder;
}
  ...
  if ( !$self->{recent_pending_runs} || $return_all ) {
    ...
    if ($return_all) {
      ...
      my $runs = [
        sort     { $a->id_run() <=> $b->id_run() }
            grep { defined $_ }
            map { ( $_, $_->run_pair() ? $_->run_pair() : undef ) }
            @{$db_runs}
      ];
...
sub run_pair {
  ...
  if ( !$self->{run_pair} ) {
    ...
    $self->{run_pair}
        = $self->gen_getarray( $pkg, $query, $self->id_run_pair(),
      $self->id_run() )->[0];
  }
