...
has sleep_interval =>
    ( q(is) => q(ro), q(isa) => q(Int), default => $DEFAULT_SLEEP );
...
sub process {
  ...
  while ( my $r = $rs->next ) {
    ...
    my $o = $ss->output;
    if ( -e $o ) { $self->log->debug(qq($o already exists)) }
    else {
      try {
        $ss->process;
        ...
        $self->log->error( qq(Trying to create $o for run )
              . ( $r->id_run )
