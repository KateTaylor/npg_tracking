...
sub render {
  ...
  print {*STDERR} qq[Serving error:\nerrstr:     ]
    ...
    . @{ [ $EVAL_ERROR || q[undef] ] }
    . qq[\nTemplate:   ]
    . @{ [ Template->error() || q[undef] ] } . qq[\n]
    or croak 'unable to print error';

  if ( $aspect eq 'add' ) {
    $errstr .=
        q(<br /><br />You do not have sufficient permissions to )
  ...
  if ( $aspect ne 'add' ) {
    ...
    if ( @{$users} ) {
      ...
      return
        ...
        . $self->actions()
        . q(<div class="error_box">)
        . q(<h3>Should it reoccur please report the matter to <a href="mailto:seq-help@)
        . q(sanger.ac.uk">seq-help@)
        . q(sanger.ac.uk</a></h3><p>)
        . $errstr
        . q(</p></div>);
    }
  }
