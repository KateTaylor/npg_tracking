...
sub authorised {
  ...
  #
  if ( ( $aspect eq 'add' || $aspect eq 'add_pair_ajax' )
    && $requestor->is_member_of('loaders') )
  {
    return 1;
  }

  if ( $aspect ne 'update_tags'
    && $action eq 'create'
    && $requestor->is_member_of('loaders') )
  {
    return 1;
  ...
  if (
    $aspect eq q{list_stuck_runs}
    && ( $requestor->is_member_of(q{loaders})
      || $requestor->is_member_of(q{annotators})
      || $requestor->is_member_of(q{manual_qc}) )
    )
  {
    return 1;
  }
...
sub list {
  ...
  my $batch_id = $cgi->param('batch_id');
  my $id_rsd =
    $cgi->param('id_run_status_dict');    # || $session->{id_run_status_dict};
...
sub list_xml {
  ...
  if ( scalar @id_runs ) {
    ...
    $model->{runs} =
      [ map { npg::model::run->new( { util => $util, id_run => $_, } ) }
        @id_run ];
    #########
...
sub create {
  ...
  my %read_cycle_count =
    map { $_ => $cgi->param( 'read_cycle_' . $_ ) }
    map { my ($c) = $_ =~ /^read_cycle_(\d+)/smx }
...
sub update_tags {
  ...
  if ( $cgi->param('tags') ) {
    @tags = map { lc $_ } split q{ }, $cgi->param('tags');
  }
  if ( $cgi->param('tagged_already') ) {
    @specified_tags = map { lc $_ } split q{ }, $cgi->param('tagged_already');
  }
  ...
  if ( !$cgi->param('allow_verified') ) {
    ...
    if ( scalar keys %verified_tags ) {
      croak
q{You cannot verify a run/flowcell/reagent using the tagging system.<br />Please go back and verify on an instrument using the button};
    }
  }
  ...
  eval {
    ...
    croak $EVAL_ERROR
      . q{rolled back attempt to save info for the tags for run }
      . $self->model->id_run();
  };
