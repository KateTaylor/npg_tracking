...
sub list {
  ...
  $model->{instruments} = [
    map { $_->[0] } sort {
        ...
        or ( $a->[3] <=> $b->[3] )
        or ( $a->[2] cmp $b->[2] )
      } map { [ $_, $_->id_instrument_format, $_->name, $_->name =~ /(\d+)/smx ] }
      @{ $model->{instruments} }
  ];
...
sub _progress_bar_image {
  ...
  if ( $current_run or defined $complete ) {
    if ( ( $run_status eq 'run in progress' ) or ( defined $complete ) ) {
      ...
      my $acc =
        ( defined $complete ) ? $complete : $current_run->actual_cycle_count();
      ...
      $im->filledRectangle(
        $x_offset,              $FILLED_RECTANGLE_TWO, $x_offset + $fraction,
        $FILLED_RECTANGLE_FOUR, $colours->{'pbar_fg'}
      );
      $im->string(
        $font,
        $x_offset,
...
# if instrument has not been connected to  th threshold duration add disconnect icon
  ...
  if ( $ins_status =~ /down/xms ) {
    my $sign_colour =
      $ins_status =~ /service/xms ? $colours->{'orange'} : $colours->{'red'};
