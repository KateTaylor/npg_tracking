...
sub list {
  ...
  $model->{instruments} = [
    map { $_->[0] } sort {
      ...
      [ $_, $_->id_instrument_format, $_->name, $_->name =~ /(\d+)/smx ]
        } @{ $model->{instruments} }
  ];
...
sub list_edit_statuses {
  ...
  my $root_isd
      = npg::model::instrument_status_dict->new( { util => $self->util(), } );
  $self->model->{instrument_status_dicts}
      = $root_isd->instrument_status_dicts();

  my $root_imd
      = npg::model::instrument_mod_dict->new( { util => $self->util(), } );
...
sub read {    ## no critic (ProhibitBuiltinHomonyms)
  ...
  my $id_rsd
      = $cgi->param('id_run_status_dict')
      || $session->{id_run_status_dict}
      || 'all';
  my $all_rsd = npg::model::run_status_dict->new( { util => $util, } )
      ->run_status_dicts();
  ...
  $model->{count_runs} = $model->count_runs(
    { id_run_status_dict => ( $id_rsd ne 'all' ) ? $id_rsd : undef } );
...
sub read_key_png {
  ...
  for my $type ( @{$TYPES} ) {
    ...
    $im->filledRectangle( $KEY_OFFSET_X, $y, $KEY_BLOCK_WIDTH,
      $y + $KEY_BLOCK_HEIGHT, $v );
    $im->string( $font, $KEY_OFFSET_X * 2 + $KEY_BLOCK_WIDTH,
      $y, $k, $colours->{'black'} );
    $y += $KEY_OFFSET_Y;
  }
...
sub _read_png_colour {
  ...
  elsif ( $statuses->{instrument} eq 'down'
    || $statuses->{instrument} eq 'down for repair' )
  {
    $bg = $colours->{'red'};
  }
  elsif ( $statuses->{instrument} eq 'planned maintenance'
    || $statuses->{instrument} eq 'planned repair' )
  {
    $bg = $colours->{'pink'};
  }
...
sub _run2image {
  my (
    $self,       $im,         $colours,  $font, $current_run,
    $run_status, $ins_status, $complete, $x_offset
  ) = @_;

  if ( !defined $x_offset ) { $x_offset = 0; }

  my $bg = $self->_read_png_colour( $im, $colours,
    { instrument => $ins_status, run => $run_status } );
  $im->filledRectangle( $x_offset, $FILLED_RECTANGLE_INFO,
    $x_offset + $IMAGE_DIMENSIONS,
    $IMAGE_DIMENSIONS, $bg );
  ...
  my $first_line
      = $npg::model::instrument_status_dict::SHORT_DESCRIPTIONS{$ins_status};
  if ( !$first_line ) {
    carp
        "WARNING: No short description for instrument status \"$ins_status\"";
  ...
  $im->string( $font, $x_offset + $x_start,
    $RUN_NAME_VALUE_TWO, $first_line, $colours->{'black'} );
  ...
  if ($run_status) {
    ...
    $im->string( $font, $x_offset + $x_start,
      $RUN_STATUS_VALUE_TWO, $run_status, $colours->{'black'} );
  }
  return;
}
...
sub _batch2image {
  ...
  $im->string( $font, $batch_x_start, $batch_y_start,
    ( sprintf q(B%d), $current_run->batch_id() ), $black );

  if ( $current_run->is_paired() ) {
    $im->string( $font, $batch_x_start, $END_TWO,
      ( sprintf q(Run %d), $current_run->end() ), $black );
  }
  else {
    ...
    if ( defined $is_paired_read ) {
      ...
      if ( $is_paired_read == 1 ) {
        $im->string( $font, $batch_x_start + $PAIRED_ONE,
          $y_coord, 'paired', $black );
      }
      else {
        $im->string( $font, $batch_x_start + $PAIRED_ONE,
          $y_coord, 'single', $black );
      }
    }
  }
  return;
...
sub _progress_bar_image {
  my ( $self, $im, $colours, $font, $current_run, $run_status, $complete,
    $x_offset )
      = @_;
  ...
  if ( $current_run or defined $complete ) {
    if ( ( $run_status eq 'run in progress' ) or ( defined $complete ) ) {
      my $ecc
          = ( defined $complete )
          ? $HUNDRED_PERCENT
          : $current_run->expected_cycle_count();
      my $acc
          = ( defined $complete )
          ? $complete
          : $current_run->actual_cycle_count();
      my $fraction = $ecc ? $acc * $IMAGE_DIMENSIONS / $ecc : 0;
      $im->filledRectangle( $x_offset, $FILLED_RECTANGLE_TWO,
        $x_offset + $IMAGE_DIMENSIONS,
        $FILLED_RECTANGLE_FOUR, $colours->{'pbar_bg'} );
      $im->filledRectangle(
        $x_offset,             $FILLED_RECTANGLE_TWO,
        $x_offset + $fraction, $FILLED_RECTANGLE_FOUR,
        $colours->{'pbar_fg'}
      );
      $im->string(
        $font,
        $x_offset,
        $FILLED_RECTANGLE_TWO,
        ( defined $complete ? sprintf q(%2d%%), $acc : sprintf q(%2d/%2d),
          $acc, $ecc
        ),
        $colours->{'black'}
      );
...
sub _run_status4image {
  my ( $self, $run ) = @_;
  return $run
      ? $run->current_run_status->run_status_dict->description()
      : q();
}
...
sub read_png {    ## no critic (Subroutines::ProhibitExcessComplexity)
  ...
  if ( !$ins_status_obj ) {
    carp 'WARNING: Current status not available for instrument '
        . $model->name;
  }
  my $ins_status
      = $ins_status_obj
      ? $ins_status_obj->instrument_status_dict->description()
      : q{?};
  my $current_run;
  my $run_status;
...
# if instrument has not been connected to  th threshold duration add disconnect icon
  if ( my $lc = $model->latest_contact ) {
    if ( DateTime::Format::MySQL->parse_datetime($lc)
      ->add( minutes => $ALERT_MINUTES_THRESHOLD ) < DateTime->now() )
    {
      my $im_fn_ds = sprintf q(%s/gfx/disconnect.png), $util->data_path();
      my $src_ds = GD::Image->newFromPng($im_fn_ds);
      $im->copy( $src_ds, $im->width - $src_ds->width - $ALERT_MARGIN,
        $Y_ALERT, 0, 0, $src_ds->width, $src_ds->height );
    }
  }
  ...
  if ( my @r = @{ $model->current_runs } ) {
    if ( any { $_->has_tag_with_value(q(copying_problem)) } @r ) {
      ...
      my $src_ds = GD::Image->newFromPng($im_fn_ds);
      $im->copy( $src_ds, $im->width - $src_ds->width - $ALERT_MARGIN,
        $Y_ALERT_TOP, 0, 0, $src_ds->width, $src_ds->height );
    }
  }
  ...
  if ( $ins_status =~ /down/xms ) {
    ...
    $im->line(
      $x_centre - $line_half_dim,
      $y_centre - $line_half_dim,
      $x_centre + $line_half_dim,
      $y_centre + $line_half_dim,
      $sign_colour
    );
    my $circle_size = $height - $ARC_SIZE_ADJUST;
    $im->arc( $width / 2, $height / 2,
      $circle_size, $circle_size, 0, $DEGREES_IN_CIRCLE, $sign_colour );
  }
  ...
  if ( any { $_->description eq q(R&D) } @{ $model->designations() || [] } ) {
    $im->string( gdGiantFont, $instr_annot_x, $INS_NAME_VALUE_TWO * 2,
      'R&D', $colours->{'yellow'} );
  }
  ...
  if ( !$is_hs ) {
    ...
    $self->_run2image( $im, $colours, $font, $current_run, $run_status,
      $ins_status, $complete );
    $self->_progress_bar_image( $im, $colours, $font, $current_run,
      $run_status, $complete );
    ...
    foreach my $fc_slot ( sort keys %{$map} ) {
      ...
      $self->_run2image( $im, $colours, $font, $run, $r_status, $ins_status,
        $complete, $x_offset );
      $self->_progress_bar_image( $im, $colours, $font, $run, $r_status,
        $complete, $x_offset );
      my $batch_offset = $count ? -20 : -60;
      $self->_batch2image( $run, $im, $font, $colours->{'black'},
        $x_offset + $batch_offset );
