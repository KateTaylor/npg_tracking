...
subtype 'NPG_API_Request_PositiveInt' => as Int => where { $_ > 0 };
...
class_has 'save2cache_dir_var_name' => (
  isa      => 'Str',
  is       => 'ro',
  required => 0,
...
has 'max_retries' => (
  isa      => 'NPG_API_Request_PositiveInt',
  is       => 'ro',
  required => 0,
  default  => $MAX_RETRIES,
);
...
sub make {
  ...
  my $cache =
    $ENV{ $self->cache_dir_var_name } ? $ENV{ $self->cache_dir_var_name } : q[];
  ...
  if ( $method ne $DEFAULT_METHOD ) {
    ...
    $content =
      ( $cache && !$ENV{ $self->save2cache_dir_var_name } )
...
sub _create_path {
  ...
  my ($stpath) =
    $url =~ m{\Ahttps?://psd\-[^/]+(.*?)\z}xms;    # sequencescape path
  ...
  ($path) =
    $path =~ m{([/[:lower:][:digit:]_.]+)}xms;  # get rid of horrible characters
...
sub _from_cache {
  ...
  local $RS = undef;
  open my $fh, q[<], $path
    or croak qq[Error when opening $path for reading: $ERRNO];
  if ( !defined $fh ) { croak qq[Undefined filehandle returned for $path]; }
  my $content =
    defined $fh ? <$fh> : croak qq[Failed to read from an open $path: $ERRNO];
...
sub _from_web {
  ...
  if ( $path
    && $ENV{ $self->save2cache_dir_var_name }
    && $ENV{ $self->cache_dir_var_name } )
  {
    my $content;
    eval { $content = $self->_from_cache( $path, $uri ); };
    if ($content) {
      return $content;
    }
  }
  ...
  if ( defined $self->content_type
    && $self->content_type eq $DEFAULT_CONTENT_TYPE
    && $content =~ /<!DOCTYPE[ ]html/xms )
  {
...
sub _write2cache {
  ...
  open my $fh, q[>], $path
    or croak qq[Error when opening $path for writing: $ERRNO];
