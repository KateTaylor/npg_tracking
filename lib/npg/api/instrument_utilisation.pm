...
__PACKAGE__->mk_accessors( fields(), 'instruments', 'official_insts',
  'prod_insts', 'hot_spares', );
  ...
  eval {
   # clear this object of any fields which by running it is expected to populate
    ...
    if ( $self->total_insts() ) {
      ...
      my $msg =
        q{No current instruments found of type } . $self->_type() . qq{\n};
...
sub _current_instruments {
  ...
  foreach my $i ( @{$instruments} ) {
    ...
    $self->instruments()->{ $i->id_instrument() } = { total_insts => 1, };
...
sub api_instruments {
  ...
  if ( !$self->{api_instruments} ) {
    $self->{api_instruments} =
      npg::api::instrument->new( { util => $self->util() } )->instruments();
  }
...
sub order_runs_by_instrument_and_organise_time {
  ...
  foreach my $inst ( sort keys %{$instruments} ) {
    ...
    my $correct_type;
    foreach my $inst_by_type (
      @{ $self->instruments_by_type()->{ $INSTRUMENT_TYPES{ $self->_type() } } }
      )
    {
    ...
    my @temp_runs =
      sort { $a->{start} cmp $b->{start} } @{ $instruments->{$inst} };
    my $wanted_runs_for_instrument = [];

    foreach my $run (@temp_runs) {
      my ( $sy, $sm, $sd, $sh, $smin, $ss ) = $run->{start} =~
        /(\d{1,4})-(\d{1,2})-(\d{1,2})[ ](\d{1,2}):(\d{1,2}):(\d{1,2})/xms;
      my ( $ey, $em, $ed, $eh, $emin, $es ) = $run->{end} =~
        /(\d{1,4})-(\d{1,2})-(\d{1,2})[ ](\d{1,2}):(\d{1,2}):(\d{1,2})/xms;
      ...
      my $compare_end_to_three_days_ago =
        $beginning_three_days_ago->compare($end);
      next
        if ( $compare_start_to_end_yesterday == 1
        || $compare_end_to_three_days_ago == 1 );
...
# If end of run A after 3 day cutoff and $run_b loaded two days ago - full utilization between
  ...
  foreach my $inst ( sort keys %{$instruments} ) {
    foreach my $run ( @{ $instruments->{$inst} } ) {
      my ( $sy, $sm, $sd, $sh, $smin, $ss ) = $run->{start} =~
        /(\d{1,4})-(\d{1,2})-(\d{1,2})[T](\d{1,2}):(\d{1,2}):(\d{1,2})/xms;
      my ( $ey, $em, $ed, $eh, $emin, $es ) = $run->{end} =~
        /(\d{1,4})-(\d{1,2})-(\d{1,2})[T](\d{1,2}):(\d{1,2}):(\d{1,2})/xms;
      $sm = sprintf '%02d', $sm;
      $em = sprintf '%02d', $em;
...
# if the instrument already has enough seconds for a full days utilization, skip
    if ( !$instruments->{ $run->{id_instrument} }->{seconds_used} >=
      $SECONDS_IN_DAY )
    {
      next;
    }
    ...
    if ( $end_number < $two_days_ago_start_num
      || $two_days_ago_end_num < $start_number )
    {
      next;
    }
...
# if run starts before two_days_ago and ends after two_days_ago, then util is whole day, and then move on
    if ( $end_number > $two_days_ago_end_num
      && $two_days_ago_start_num > $start_number )
    {
...
# work out portion of day in secs if the run overlaps the start or end of two_days_ago
    if ( $end_number > $two_days_ago_start_num
      && $start_number < $two_days_ago_start_num )
    {

      $instruments->{ $run->{id_instrument} }->{seconds_used} +=
        $end->subtract_datetime_absolute($beginning_of_two_days_ago)->seconds();

    }
    elsif ( $start_number < $two_days_ago_end_num
      && $end_number > $two_days_ago_end_num )
    {

      $instruments->{ $run->{id_instrument} }->{seconds_used} +=
        $end_of_two_days_ago->subtract_datetime_absolute($start)->seconds();
  ...
  foreach my $i ( sort keys %{$instruments} ) {
    ...
    if ( $instruments->{$i}->{seconds_used} ) {
      ...
      $instruments->{$i}->{perc_used} =
        ( $instruments->{$i}->{seconds_used} ) * $PERCENTAGE / $SECONDS_IN_DAY;
    ...
    if ( $instruments->{$i}->{official} ) {
      $official += $instruments->{$i}->{perc_used};
    }
    if ( $instruments->{$i}->{production} ) {
      $production += $instruments->{$i}->{perc_used};
    }

  }

  if ( $self->total_insts() ) {
    $self->perc_utilisation_total_insts( sprintf '%.2f',
      ( $total / $self->total_insts() ) );
  }
  if ( $self->official_insts() ) {
    $self->perc_utilisation_official_insts( sprintf '%.2f',
      ( $total / $self->official_insts() ) );
  }
  if ( $self->prod_insts() ) {
    $self->perc_utilisation_prod_insts( sprintf '%.2f',
      ( $production / $self->prod_insts() ) );
  }
...
sub recent_running_runs {
  ...
  if ( !$self->{recent_running_runs} ) {
    ...
    $self->{recent_running_runs} =
      npg::api::run->new( { util => $self->util() } )->recent_running_runs();
...
sub id_run_pair {
  my ( $self, $id_run ) = @_;
  return npg::api::run->new( { util => $self->util(), id_run => $id_run } )
    ->id_run_pair();
}
...
sub beginning_of_yesterday_dt_object {
  ...
  if ( !$self->{beginning_of_yesterday_dt_object} ) {
    ...
    my $m  = $dt->month();
    my $d  = $dt->day();
    $self->{beginning_of_yesterday_dt_object} = DateTime->new(
      { time_zone => 'UTC', year => $y, month => $m, day => $d } );
  }
...
sub end_of_yesterday_dt_object {
  ...
  if ( !$self->{end_of_yesterday_dt_object} ) {
    ...
    my $m  = $dt->month();
    my $d  = $dt->day();
    $self->{end_of_yesterday_dt_object} = DateTime->new(
      {
        time_zone => 'UTC',
        year      => $y,
        month     => $m,
        day       => $d,
        hour      => $LAST_HOUR_OF_DAY,
        minute    => $LAST_MINUTE_OF_DAY,
        second    => $LAST_MINUTE_OF_DAY
      }
    );
  }
...
sub two_days_ago_datetime_object {
  ...
  if ( !$self->{two_days_ago_datetime_object} ) {
    $self->{two_days_ago_datetime_object} =
      DateTime->now()->subtract( days => 2 );
  }
...
sub beginning_of_two_days_ago_dt_object {
  ...
  if ( !$self->{beginning_of_two_days_ago_dt_object} ) {
    ...
    my $m  = $dt->month();
    my $d  = $dt->day();
    $self->{beginning_of_two_days_ago_dt_object} = DateTime->new(
      { time_zone => 'UTC', year => $y, month => $m, day => $d } );
  }
...
sub end_of_two_days_ago_dt_object {
  ...
  if ( !$self->{end_of_two_days_ago_dt_object} ) {
    ...
    my $m  = $dt->month();
    my $d  = $dt->day();
    $self->{end_of_two_days_ago_dt_object} = DateTime->new(
      {
        time_zone => 'UTC',
        year      => $y,
        month     => $m,
...
sub two_pm_cutoff {
  my ( $self, $day ) = @_;
  my $dt =
      $day == 2                ? $self->two_days_ago_datetime_object()
...
sub three_days_ago_datetime_object {
  ...
  if ( !$self->{three_days_ago_datetime_object} ) {
    $self->{three_days_ago_datetime_object} =
      DateTime->now()->subtract( days => $THREE_DAYS_BACK );
  }
...
sub beginning_of_three_days_ago_dt_object {
  ...
  if ( !$self->{beginning_of_three_days_ago_dt_object} ) {
    ...
    my $m  = $dt->month();
    my $d  = $dt->day();
    $self->{beginning_of_three_days_ago_dt_object} = DateTime->new(
      { time_zone => 'UTC', year => $y, month => $m, day => $d } );
  }
...
sub end_of_three_days_ago_dt_object {
  ...
  if ( !$self->{end_of_three_days_ago_dt_object} ) {
    ...
    my $m  = $dt->month();
    my $d  = $dt->day();
    $self->{end_of_three_days_ago_dt_object} = DateTime->new(
      {
        time_zone => 'UTC',
        year      => $y,
        month     => $m,
...
sub percentage_uptimes_of_all_instruments {
  ...
  foreach my $i ( sort keys %{$insts} ) {
    ...
    if ( $insts->{$i}{official} ) {
      $official += $uptime->{ $insts->{$i}{name} }->{percentage};
    }
    if ( $insts->{$i}{production} ) {
      $production += $uptime->{ $insts->{$i}{name} }->{percentage};
    }
  }

  if ( $self->total_insts() ) {
    $self->perc_uptime_total_insts( sprintf '%.2f',
      ( $total / $self->total_insts() ) );
  }
  if ( $self->official_insts() ) {
    $self->perc_uptime_official_insts( sprintf '%.2f',
      ( $total / $self->official_insts() ) );
  }
  if ( $self->prod_insts() ) {
    $self->perc_uptime_prod_insts( sprintf '%.2f',
      ( $production / $self->prod_insts() ) );
  }
...
sub two_days_ago_uptime {
  ...
  if ( !$self->{two_days_ago_uptime} ) {
    ...
    foreach my $name ( sort keys %{$uptime_per_instrument} ) {
      $uptime_per_instrument->{$name}->{percentage} = sprintf '%.2f',
        ( $uptime_per_instrument->{$name}->{seconds} *
          $PERCENTAGE /
          $SECONDS_IN_DAY );
    }
...
sub two_days_ago_uptime_in_seconds {
  ...
  if ( !$self->{two_days_ago_uptime_in_seconds} ) {
    ...
    my $m  = $dt->month();
    my $d  = $dt->day();
    my $beginning_of_two_days_ago =
      $self->beginning_of_two_days_ago_dt_object();
    my $end_of_two_days_ago = $self->end_of_two_days_ago_dt_object();
    ...
    foreach my $i ( @{$instruments} ) {
      ...
      foreach my $s (@statuses) {
        ...
        $ss   = sprintf '%02d', $ss;
        my $current_dt_object = DateTime->new(
          {
            time_zone => 'UTC',
            year      => $sy,
            month     => $sm,
        ...
        if ( $s_string < $two_days_ago_start_num )
        {    # this status date is before two_days_ago
          if ( $state eq 'up' ) {    # only worry if this state is up
            if ($last_down)
            {    ## no critic (ControlStructures::ProhibitDeepNests)
              if ( $last_down > $two_days_ago_end_num )
              {    ## no critic (ControlStructures::ProhibitDeepNests)
                ...
                $seconds_per_instrument->{$name}{seconds} +=
                  $last_dt_object->subtract_datetime_absolute(
                  $beginning_of_two_days_ago)->seconds();
              }
            }
            else {
              if ( !$last_up || $last_up > $two_days_ago_end_num )
              {         ## no critic (ControlStructures::ProhibitDeepNests)
                        # this implies it was up all day
                $seconds_per_instrument->{$name}{seconds} = $SECONDS_IN_DAY;
              }
              else
              { # whilst still up, the rest of the day would already have been counted
                $seconds_per_instrument->{$name}{seconds} +=
                  $last_dt_object->subtract_datetime_absolute(
                  $beginning_of_two_days_ago)->seconds();
              }
        ...
        elsif ( $s_string > $two_days_ago_end_num )
        { # this status date is after two_days_ago, just set the date and the state
          $current_state = $state;
          ...
          if ( $state eq 'down' && $current_state eq 'up' )
          ...
          { # if we are about to enter a period (going backwards) where it is down
            ...
            {    ## no critic (ControlStructures::ProhibitDeepNests)
              ...
              $seconds_down_til_end_of_day +=
                $end_of_two_days_ago->subtract_datetime_absolute(
                $last_dt_object)->seconds();
            }
            $seconds_per_instrument->{$name}{seconds} +=
              $end_of_two_days_ago->subtract_datetime_absolute(
              $current_dt_object)->seconds();
            $seconds_per_instrument->{$name}{seconds} -=
              $seconds_down_til_end_of_day;
            $last_down = undef;
            $last_up   = $s_string;
          ...
          { # if we have had two statuses of up (going backwards) (possible, but unlikely)
            $seconds_per_instrument->{$name}{seconds} +=
              $last_dt_object->subtract_datetime_absolute($current_dt_object)
              ->seconds();
            my $seconds_down_til_end_of_day = 0;
            if ( $last_up < $two_days_ago_end_num )
            {    ## no critic (ControlStructures::ProhibitDeepNests)
              $seconds_down_til_end_of_day +=
                $end_of_two_days_ago->subtract_datetime_absolute(
                $last_dt_object)->seconds();
          ...
          elsif ( $state eq 'down' || $state eq 'down for repair' )
          { # if we have had a second period where it has been down (possible, but unlikely)
            $last_down = $s_string;
          }
        }
...
sub instrument_status_object {
  ...
  if ( !$self->{instrument_status_object} ) {
    $self->{instrument_status_object} =
      npg::api::instrument_status->new( { util => $self->util() } );
  }
...
sub insert_date_for_record {
  my ($self) = @_;
  $self->date( $self->beginning_of_two_days_ago_dt_object()->ymd() . q{ }
      . $self->beginning_of_two_days_ago_dt_object()->hms() );
  return 1;
}
