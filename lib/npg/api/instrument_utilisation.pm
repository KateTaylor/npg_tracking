...
sub _calculate_values {
 ...
 # clear this object of any fields which by running it is expected to populate
    ...
    if ( $self->total_insts() ) {
      ...
      my $msg = q{No current instruments found of type }
          . $self->_type() . qq{\n};
      print $msg or carp $msg;
    }
    1;
  } or do {
    croak q{Unable to calculate }
        . $self->_type()
        . q{ values: }
        . $EVAL_ERROR;
  };
...
sub api_instruments {
  ...
  if ( !$self->{api_instruments} ) {
    $self->{api_instruments}
        = npg::api::instrument->new( { util => $self->util() } )
        ->instruments();
  }
...
sub order_runs_by_instrument_and_organise_time {
  ...
  my $beginning_three_days_ago
      = $self->beginning_of_three_days_ago_dt_object();
...
# work out portion of day in secs if the run overlaps the start or end of two_days_ago
      ...
      && $start_number < $two_days_ago_start_num )
    {
      ...
      $instruments->{ $run->{id_instrument} }->{seconds_used}
          += $end->subtract_datetime_absolute($beginning_of_two_days_ago)
          ->seconds();
      ...
      && $end_number > $two_days_ago_end_num )
    {
      ...
      $instruments->{ $run->{id_instrument} }->{seconds_used}
          += $end_of_two_days_ago->subtract_datetime_absolute($start)
          ->seconds();
...
sub recent_running_runs {
  ...
  if ( !$self->{recent_running_runs} ) {
    ...
    $self->{recent_running_runs}
        = npg::api::run->new( { util => $self->util() } )
        ->recent_running_runs();
