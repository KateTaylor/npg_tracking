...
sub table_portrait {
  ...
  my $height =
    ...
    $BOTTOM_MARGIN_TP;
  my $width =
    $cols * $width_of_image + $WIDTH_OF_ROW_HEADINGS_TP + $RIGHT_MARGIN_TP;
...
sub table_landscape {
  ...
  my $height =
    ...
    $BOTTOM_MARGIN_TL;
  my $width =
    $cols * $width_of_image + $WIDTH_OF_ROW_HEADINGS_TL + $RIGHT_MARGIN_TL;
...
sub all_error_thumbs {
  ...
  foreach my $lane ( 1 .. $max_lane ) {
    ...
    foreach my $tile ( 1 .. $max_tile ) {
      ...
      my $tile_ref = $arg_refs->{id_run} . q{_} . $lane . q{_} . sprintf '%03d',
        $tile;
  ...
  my $cmh =
    scalar @{$rows} * 2 * ( $args_for_image->{height} + $EXTRA_FIVE_PIXELS ) +
    $HEIGHT_OF_COLUMN_HEADINGS_TL;
  ...
  foreach my $row ( @{$rows} ) {
    ...
    if ( ref $row->[$IMAGE_ALL_ERROR_THUMBS] eq 'GD::Image' ) {
      ...
      my $x2 =
        $S_LESS_ONE_BORDER_TILE_REF_ALL_ERROR_THUMBS + $args_for_image->{width};
...
sub error_add_text {
  ...
  $im->string( gdGiantFont, $EXTRA_FIVE_PIXELS, $EXTRA_FIVE_PIXELS, 'Tile Ref',
    $text_colour );
