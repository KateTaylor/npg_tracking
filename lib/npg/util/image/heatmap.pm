...
sub plot_illumina_map {
  ...
  my $white = $im->colorAllocate( $ALLOCATE_WHITE, $ALLOCATE_WHITE,
    $ALLOCATE_WHITE );
  $im->transparent($white);

  my $data_gradient = $self->get_data_gradient( $im, $data_array, $arg_refs,
    $tile_layout_info );
...
sub get_data_gradient {
  ...
  my $data_gradient
      ...
      && $arg_refs->{gradient_style} eq 'percentage'
    ...
    $arg_refs->{colours}, $PERC_ERROR_RANGE_MAX )
      : $arg_refs->{gradient_style}
      && $arg_refs->{gradient_style} eq 'cluster'
      ? $self->give_datum_a_range_gradient( $im, $data_array,
    $arg_refs->{colours}, $tile_layout_info->{CLUSTER_MAX} )
      : $arg_refs->{gradient_style}
      && $arg_refs->{gradient_style} eq 'intensity'
      ? $self->give_datum_a_range_gradient( $im, $data_array,
    $arg_refs->{colours}, $tile_layout_info->{INTENSITY_MAX} )
      ...
      : $self->give_datum_a_gradient( $im, $data_array,
    $arg_refs->{colours} );
...
sub get_movez_heatmap_color {
  ...
  my $grey = $im->colorAllocate( $GREY_COLOR_INDEX, $GREY_COLOR_INDEX,
    $GREY_COLOR_INDEX );
