...
sub plot_illumina_map {
  ...
  my $white =
    $im->colorAllocate( $ALLOCATE_WHITE, $ALLOCATE_WHITE, $ALLOCATE_WHITE );
  $im->transparent($white);

  my $data_gradient =
    $self->get_data_gradient( $im, $data_array, $arg_refs, $tile_layout_info );
  ...
  if ( $arg_refs->{vertical} ) {
    ...
    foreach my $i ( 1 .. $NUMBER_OF_LANES ) {
      ...
      $im->string( gdSmallFont, $x_position_of_number, $y_position_of_number,
        $i, $text_colour );
...
sub get_data_gradient {
  ...
  my $data_gradient =
    $arg_refs->{gradient_style} && $arg_refs->{gradient_style} eq 'percentage'
    ? $self->give_datum_a_range_gradient( $im, $data_array,
    $arg_refs->{colours}, $PERCENTAGE_RANGE_MAX )
    : $arg_refs->{gradient_style}
    && $arg_refs->{gradient_style} eq 'percentage_error_rates'
    ? $self->give_datum_a_range_gradient( $im, $data_array,
    $arg_refs->{colours}, $PERC_ERROR_RANGE_MAX )
    : $arg_refs->{gradient_style} && $arg_refs->{gradient_style} eq 'cluster'
    ? $self->give_datum_a_range_gradient( $im, $data_array,
    $arg_refs->{colours}, $tile_layout_info->{CLUSTER_MAX} )
    : $arg_refs->{gradient_style} && $arg_refs->{gradient_style} eq 'intensity'
    ? $self->give_datum_a_range_gradient( $im, $data_array,
    $arg_refs->{colours}, $tile_layout_info->{INTENSITY_MAX} )
    : $arg_refs->{gradient_style} && $arg_refs->{gradient_style} eq 'movez'
    ? $self->get_movez_heatmap_color($im)
...
sub give_datum_a_range_gradient {
  ...
  foreach my $tile_value ( sort { $a <=> $b } keys %{$all_data_points} ) {
    ...
    foreach my $i ( 0 .. $gradients ) {
      ...
      if ( $tile_value >= $bin_starts[$i]->[0]
        && ( !$bin_starts[$a] || $tile_value <= $bin_starts[$a]->[0] ) )
      {
...
sub get_movez_heatmap_color {
  ...
  my $grey =
    $im->colorAllocate( $GREY_COLOR_INDEX, $GREY_COLOR_INDEX,
    $GREY_COLOR_INDEX );
