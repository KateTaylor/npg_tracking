...
sub plot_scale {
  ...
  my $defaults
      = $arg_refs->{orientation} eq 'vertical'
      ? $VERTICAL_DEFAULTS
      : $HORIZONTAL_DEFAULTS;
  ...
  my $linear_gradient
      = $self->build_linear_gradient( $arg_refs->{colours}, q{},
    $arg_refs->{gradient_steps}, q{scale} );
  ...
  my $all_data_points
      = $self->give_pixels_a_gradient( $im, $max_pixels, $linear_gradient );

  my $gap_width
      = $arg_refs->{orientation} eq 'vertical'
      ? sprintf '%.00f', ( ( $image_width - $bar_width ) / $QUARTER_DIVISION )
      : sprintf '%.00f', ( ( $image_width - $bar_width ) / $HALF_DIVISION );

  my $gap_height
      = $arg_refs->{orientation} eq 'vertical'
      ? sprintf '%.00f', ( ( $image_height - $bar_height ) / $HALF_DIVISION )
      : sprintf '%.00f',
      ( ( $image_height - $bar_height ) / $QUARTER_DIVISION );

  if ( $arg_refs->{orientation} eq 'vertical' ) {

    $im->string( gdSmallFont, ( 2 * $gap_width ),
      $gap_height, $arg_refs->{start_text}, $text_colour );
    $im->string( gdSmallFont,
      ( 2 * $gap_width ),
      ( $image_height - $gap_height - $GDSMALL_FONT_PIXELS ),
      $arg_refs->{end_text}, $text_colour
    );

  }
  else {

    $im->string( gdSmallFont, $gap_width, $gap_height,
      $arg_refs->{start_text}, $text_colour );

    my $multiplier
        = length $arg_refs->{end_text}
        > $MULTIPLIER_FOR_HORIZONTAL_POSITION_OF_END_TEXT
        ? ( length $arg_refs->{end_text} ) - 1
        : $MULTIPLIER_FOR_HORIZONTAL_POSITION_OF_END_TEXT;

    $im->string( gdSmallFont, ( $image_width - $multiplier * $gap_width ),
      $gap_height, $arg_refs->{end_text}, $text_colour );
