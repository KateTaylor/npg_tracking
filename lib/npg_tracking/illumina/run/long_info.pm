...
sub _build__runinfo_store {
  ...
  if ( not defined $formatversion ) {
    ...
    my $indexprepelementfound;
    my @nodelist =
      $doc->getElementsByTagName('Reads')->[0]->getElementsByTagName('Read');
    foreach (@nodelist) {
    ...
    my $ncol =
      $fcl_el->getAttribute('SurfaceCount') *
...
sub _build__tilelayout_store {
  ...
  my $tilelayout =
    io( join q(/), $self->runfolder_path, 'Config', 'TileLayout.xml' )->slurp;
...
sub _build_tile_count {
  ...
  try {
    ...
    my $lane_el =
      $self->data_intensities_config_xml_object()->getElementsByTagName('Lane')
      ->[0];
...
sub _build_data_intensities_config {
  my ($self) = @_;
  my $c =
    io( join q(/), $self->runfolder_path, qw(Data Intensities config.xml) )
    ->slurp;
