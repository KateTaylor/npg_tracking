...
has q{index_length} => (
  ...
  lazy_build => 1,
  documentation =>
      q{The length of the index 'barcode', normally the number of cycles performed},
);
...
has q{use_bases} => (
  ...
  lazy_build => 1,
  documentation =>
      q{The config string used by Illumina and other scripts to describe the run i.e. Y54,I6n,y54},
);
...
sub _build_use_bases {
  ...
  my $l      = 0;
  return join q(,),
      pairwise { ( $b ? q(I) : ( $l++ ? q(y) : q(Y) ) ) . $a } @rc, @ri;
}
...
sub _fetch_recipe {
  my $self = shift;
  my @files = map { $self->runfolder_path . qq(/$_) }
      map {/Recipe\S*?[.]xml/smxg} io( $self->runfolder_path )->all;
  if ( @files < 1 ) {
...
sub _build__recipe_store {
  ...
  $self->_set_expected_cycle_count(
    sum map { $_->getElementsByTagName('Incorporation')->size() } @nodelist );
  ...
  foreach ( map { $_->getElementsByTagName(q(*)) } @nodelist ) {
    ...
    if ( $_->localname eq 'Incorporation' ) {
      ...
      if ( $_->getAttribute('Name') =~ /(\AEnd)|(FirstBase\Z)/smx ) {
        ...
        if ( $indexprepelementfound
          or ( $_->getAttribute('Name') =~ /\AIndexing/smx ) )
        {
...
sub _build__runinfo_store {
  ...
  my $formatversion
      = $doc->getElementsByTagName('RunInfo')->[0]->getAttribute('Version');
  if ( not defined $formatversion ) {
    $self->_set_lane_count( $doc->getElementsByTagName('Lane')->size );
    $self->_set_expected_cycle_count(
      $doc->getElementsByTagName('Cycles')->[0]->getAttribute('Incorporation')
    );
    ...
    my $indexprepelementfound;
    my @nodelist = $doc->getElementsByTagName('Reads')->[0]
    ...
    my $ncol = $fcl_el->getAttribute('SurfaceCount')
        * $fcl_el->getAttribute('SwathCount');
    ...
    if ( $rc->{indexingcurrent} ) {
      ...
      if ( $self->has_indexing_cycle_range ) {
        ...
        if ( $end != $rc->{start} + 1 ) {
          ...
          carp
              "Don't know how to deal with no adjacent indexing reads: $start,$end and $rc->{start},$rc->{index}";
        }
      }
      else {
      ...
      if ( $rc->{read_index} == 1 ) {
        ...
        carp
            "Don't know how to deal with more than 2 non index reads (read index $rc->{read_index}, last read range $rc->{start},$rc->{index})";
      }
...
foreach my $f (
  qw(expected_cycle_count lane_count read_cycle_counts indexing_cycle_range read1_cycle_range read2_cycle_range)
    )
{
  before $f => sub {
    my $self = shift;
...
sub _build__tilelayout_store {
  ...
  my $tilelayout
      = io( join q(/), $self->runfolder_path, 'Config', 'TileLayout.xml' )
      ->slurp;
...
sub _build_tile_count {
  ...
  try {
    ...
    my $lane_el = $self->data_intensities_config_xml_object()
        ->getElementsByTagName('Lane')->[0];
...
sub _build_data_intensities_config {
  my ($self) = @_;
  my $c = io( join q(/), $self->runfolder_path,
    qw(Data Intensities config.xml) )->slurp;
...
sub _build_lane_tile_clustercount {
  ...
  my $run_el
      = (
    $self->data_intensities_config_xml_object()->getElementsByTagName('Run') )
      [0];

  my $lane_tile_clustercount = {};
  for my $lane_el ( ( $run_el->getChildrenByTagName('TileSelection') )[0]
    ->getChildrenByTagName('Lane') )
  {
