...
subtype 'NPG_TRACKING_REFERENCE_REPOSITORY' => as Str => where { -d $_ };
...
sub _build_organisms {
  ...
  opendir my $dh, $self->ref_repository
      or croak q[Cannot get listing on the known organisms, cannot open ]
      . $self->ref_repository;
  ...
  foreach my $item ( grep !/^[.]/smx, @listing ) {
    if (
         $item eq $self->taxons_dir
      || $item eq $BIN_DIR
      || (
        !$self->all_species && grep /^$item$/smx,
        @{ $self->optional_species }
      )
        )
    {
      next;
  ...
  if ( @orgs == 0 ) {
    croak q[Empty listing for directory (not counting upward links) ]
        . $self->ref_repository;
  }
  return \@orgs;
}
...
sub _build_repository_contents {
  ...
  opendir my $dh, $dir
      or croak q[Cannot get listing of known taxons, cannot open ] . $dir;
  my @listing = readdir $dh;
  closedir $dh or carp q[Cannot close a dir handle];
  ...
  foreach my $taxon_id ( grep !/^[.]/smx, @listing ) {
    ...
    if ( !exists $description->{strain} ) {
      my $deafult_strain_path
          = catfile( $self->ref_repository, $description->{species},
        q[default] );
      if ( !-e $deafult_strain_path ) {
        croak qq[Taxon id $taxon_id: no default strain link in ]
            . catfile( $self->ref_repository, $description->{species} );
      }
  ...
  foreach my $sp ( @{ $self->organisms } ) {
    ...
    opendir my $dh, $sdir
        or croak q[Cannot get listing for a directory, cannot open ] . $sdir;
...
sub bait_report {
  ...
  foreach my $bait_name ( sort keys %{ $self->bait_repository_contents } ) {
    $report
        .= $bait_name . qq[\t]
        . join( q[ ], @{ $self->bait_repository_contents->{$bait_name} } )
        . qq[\n];
  }
  return $report;
}
...
sub report {
  ...
  my $list = join $REPORT_DELIM,
      ( 'Species:Strain', 'Is Default?', 'Taxon Ids', 'Synonyms' );
  $list .= "\n";
  ...
  foreach my $key ( sort keys %{$report} ) {
    $list
        .= $key . $REPORT_DELIM . $report->{$key}->{default} . $REPORT_DELIM;
  ...
  if ($filename) {
    open my $fh, q[>], $filename
        or croak qq[Cannot open file $filename for writing.];
    if ($fh) {
...
sub ref_file_prefix {
  ...
  croak
      qq[Reference file with .fa or .fasta or .fna extension not found in $fasta_dir];
}
