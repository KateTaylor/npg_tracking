...
sub BUILD {
  my $self = shift;
  if ( $self->position
    && !exists $self->data->{$DATA_SECTION}->{ $self->position } )
  {
    croak sprintf 'Position %s not defined in %s', $self->position,
        $self->path;
  }
  if ( $self->tag_index
    && !
    ...
    ->{ $self->tag_index } )
  {
    croak sprintf 'Tag index %s not defined in %s', $self->tag_index,
        $self->path;
  }
  return;
}
...
sub _build_data {
  ...
  foreach my $line (@lines) {
    ...
    my @columns = split $SAMPLESHEET_RECORD_SEPARATOR, $line,
        $RECORD_SPLIT_LIMIT;
    my @empty
        = grep { ( !defined $_ || $_ eq q[] || $_ =~ /^\s+$/smx ) } @columns;
    ...
    if ( $current_section eq 'Data' ) {
      ...
      if ($index) {
        ...
        $index
            = $d->{$current_section}->{ $lanes[0] }
            ? scalar keys %{ $d->{$current_section}->{ $lanes[0] } }
            : 0;
        $index += 1;
      }
      else {
      ...
      foreach my $lane (@lanes) {    #give all lanes explicitly
        if ( exists $d->{$current_section}->{$lane}->{$index} ) {
          croak
              "Multiple $current_section section definitions for lane $lane index $index";
        }
      ...
      if ( scalar @row > 2 ) {
        croak
            "More than two columns defined in one row in section $current_section";
      }
...
sub _build_is_pool {
  my $self = shift;
  if (
    $self->position && !$self->tag_index &&    #lane level or tag zero
    ( !exists $self->data->{$DATA_SECTION}->{ $self->position }
      ->{$NOT_INDEXED_FLAG}
    )
      )
...
sub _build__data_row {
  ...
  if ( $self->position ) {
    if ( $self->tag_index ) {
      ...
      return $self->data->{$DATA_SECTION}->{ $self->position }
          ->{ $self->tag_index };
    }
  ...
  __PACKAGE__->meta->add_method(
    ...
    sub {
      ...
      if ( $self->_data_row ) {
        ...
        elsif ( $m eq 'sample_name'
          && ( !exists $self->_data_row->{$column_name} ) )
        {
