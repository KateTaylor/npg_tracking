...
sub _build__associated_lims {
  ...
  if ( !defined $self->position ) {
    foreach my $lane_el ( @{ $self->_lane_elements } ) {
      ...
      if ( !$position ) {
        croak q[Position is not defined for one of the lanes in batch ]
            . $self->batch_id;
      }
...
sub _build__entity_xml_element {
  ...
  my $element;
  my $is_control
      = $self->_lane_xml_element->getChildrenByTagName(q[control]) ? 1 : 0;
  my $has_pool_element
      = $self->_lane_xml_element->getChildrenByTagName(q[pool]) ? 1 : 0;
  my $is_pool = !defined $self->tag_index && $has_pool_element ? 1 : 0;

  my $plexes
      = $has_pool_element
      ? $self->_lane_xml_element->getElementsByTagName(q[sample])
      : undef;
  if ( $self->tag_index ) {
    if ( !$plexes || !@{$plexes} ) {
      croak 'No plexes defined for lane '
          . $self->position
          . q[ in batch ]
          . $self->batch_id;
    }
      ...
      foreach my $plex ( @{$plexes} ) {
        ...
        if ($iel) {
          ...
          if ( $self->tag_index == $index ) {
            $element = $plex;
            $is_control
                ||= $plex->parentNode->nodeName() eq q{hyb_buffer} ? 1 : 0;
            last;
          }
        }
      }
    ...
    if ( !$element ) {
      my $buffer
          = $self->_lane_xml_element->getChildrenByTagName(q[hyb_buffer]);
      if ($buffer) {
        $buffer = $buffer->[0];
...
sub _build__subentity_xml_element {
  ...
  if (!$self->_entity_xml_element
    || $self->is_pool
    || defined $self->tag_index )
  {
    return;
  }
  my $subentity
      = $self->_entity_xml_element->getChildrenByTagName(q[sample])->[0]
      || undef;
  return $subentity;
}
...
sub _build_default_tag_sequence {
  ...
  if ( $self->tag_index ) {
    if ( !$seq && $self->_entity_xml_element ) {
      my $sel = $self->_entity_xml_element->getElementsByTagName(
        q[expected_sequence]);
      if ($sel) {
...
sub _build_spiked_phix_tag_index {
  ...
  if ( $self->_lane_xml_element ) {
    my $buffer
        = $self->_lane_xml_element->getElementsByTagName(q[hyb_buffer]);
    if ($buffer) {
...
sub _build_required_insert_size_range {
  ...
  if ( !$self->is_control ) {
    my $is_element
        = $self->_entity_xml_element->getElementsByTagName(q[insert_size]);
    if ($is_element) {
...
has 'study_contains_nonconsented_human' => (
  isa        => 'Bool',
  is         => 'ro',
  init_arg   => undef,
  lazy_build => 1,
...
has 'study_contains_nonconsented_xahuman' => (
  isa        => 'Bool',
  is         => 'ro',
  init_arg   => undef,
  lazy_build => 1,
