    ...
    lives_ok {
        $test =
          Moose::Meta::Class->create_anon_class(
            roles => [qw(npg_tracking::illumina::run)], )->new_object();
    }
    'anon class with no schema method';
    throws_ok { $test->tracking_run } qr/Need NPG tracking schema/,
      ' arrrgh no schema method'
}
...
{    # schema defined, no id_run method
    ...
    throws_ok { $test->tracking_run } qr/Need id_run/,
      ' arrrgh no id_run method';
}
...
{    # schema defined, id_run undef
    ...
    $testc->add_attribute(
        q[npg_tracking_schema] => ( q[is] => q[ro], q[default] => undef ) );
    $testc->add_attribute(
        q[id_run] => ( q[is] => q[ro], q[default] => undef ) );
    my $test;
    lives_ok {
        $test = $testc->new_object( q[npg_tracking_schema] => $testschema );
    }
...
{    # schema defined, id_run defined
    ...
    ' lives and gives npg_tracking::Schema::Result::Run';
}
