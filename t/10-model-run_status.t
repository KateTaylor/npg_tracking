...
);
{
    ...
    is_deeply( $rs->_runs_at_requested_statuses(),
        $expected_data,
        q{correct data from _runs_at_requested_statuses obtained} );
    ...
    $expected_data = {
        'analysis complete' => [
            { 'days' => 1364, 'id_run' => '1',  priority => 1 },
            { 'days' => 1364, 'id_run' => '95', priority => 4 },
        ],
        'run complete' => [
            { 'days' => 423, 'id_run' => '9948', priority => 1 },
            { 'days' => 423, 'id_run' => '9949', priority => 1 },
            { 'days' => 423, 'id_run' => '9950', priority => 1 },
            { 'days' => 423, 'id_run' => '9951', priority => 1 },
        ],
        'run mirrored' => [
            { 'days' => 1364, 'id_run' => '5',  priority => 1 },
            { 'days' => 1364, 'id_run' => '11', priority => 1 },
            { 'days' => 1364, 'id_run' => '12', priority => 1 },
        ]
    };

    is_deeply( $rs->potentially_stuck_runs(),
        $expected_data, q{potentially_stuck_runs returns the expected data} );
}
...

{
    ...
    is( ( scalar @{ $rs->current_run_statuses() } ),
        14, 'current run_statuses' );
    isa_ok( $rs->current_run_statuses->[0],
        'npg::model::run_status', 'first current run status isa_ok' );
}
...

{
    ...
    is( ( scalar @{ $rs->active_runs_over_last_30_days() } ),
        0, 'no recent active runs' );
}
...

{
    ...
    isa_ok( $potentially_stuck_runs, q{HASH},
        q{$model->potentially_stuck_runs()} );
}
