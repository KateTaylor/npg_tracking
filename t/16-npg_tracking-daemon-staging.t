...

{
    ...
    my $r       = npg_tracking::daemon::staging->new(
        timestamp => '2013',
        hosts     => [ 'sf2-nfs', 'sf3-nfs' ]
    );
    throws_ok { $r->command } qr/Need host name/,
    ...
    is(
        $r->start(q[sf2-nfs]),
qq[daemon -i -r -a 10 -n staging_area_monitor --umask 002 -A 10 -L 10 -M 10 -o $log_dir/staging_area_monitor-sf2-nfs-2013.log -- $script /export/sf2],
        'start command'
    );
    is(
        $r->ping,
q[daemon --running -n staging_area_monitor && echo -n 'ok' || echo -n 'not ok'],
        'ping command'
    );
