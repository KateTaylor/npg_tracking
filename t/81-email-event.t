...

{
    ...
    isa_ok( $notify,
        q{npg::email::event::} . $event_type . q{::} . $temp_entity_type,
        q{$notify} );
    ...
    ok( $notify,
        q{found object as legitimate event_type and entity_type provided} );
    isa_ok( $notify,
        q{npg::email::event::} . $event_type . q{::} . $temp_entity_type,
        q{$notify} );
...
q{object creation ok with suitable values (status change, instrument_status) in event_row};
    ok( $notify, q{found object as legitimate values in event_row provided} );
    isa_ok( $notify, q{npg::email::event::} . $event_type . q{::instrument},
        q{$notify} );
