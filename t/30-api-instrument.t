...

{
    ...
    isa_ok( $inst->{designations}, 'ARRAY',
        'list of designations obtained via list' );
    my $desig = $inst->designations()->[0];
    is( $desig->description(), 'Hot spare',
        'designation description obtained ok' );
    $desig = $inst->designations()->[1];
    is( $desig->description(), 'Accepted',
        'designation description obtained ok' );
    ...
    is( $cis->instrument->id_instrument(),
        1, 'cis -> instrument id_instrument' );

    my $ciss = $instrument->instrument_statuses();
    is( scalar @{$ciss}, 1, 'correct number of statuses' );

    is(
        $cis->id_instrument_status(),
        $ciss->[0]->id_instrument_status(),
        'current_ins_stat == instrument_statuses->[n]'
    );
    ...
    is(
        ...
        'ids match between instrument and instrument_status->instrument'
    );
    ...
    cmp_bag(
        $designation,
        [ 'R&D', 'Hot spare' ],
        'retrieve multiple designations'
    );
}
