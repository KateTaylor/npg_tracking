...
like(
    $EVAL_ERROR,
    qr{Given version number is not valid:},
    'no valid version given'
);
...
is( $model->runs_on_batch(10),
    $runs_on_batch, '$model->runs_on_batch(10) cached ok' );
...
my $name = $model;
is( $name->name(), 'UNKNOWN_00000',
    'unknown name if no instrument name and no id_run - default' );

$model->instrument_format->model('HK');
is( $name->name(), 'UNKNOWN_0000',
    'unknown name if no instrument name and no id_run - HK model' );

$model->{scs28} = 1;
is( $name->name(), 'UNKNOWN_00000',
    'unknown name if no instrument name and no id_run - HK model, scs28 mod' );

is( $model->attach_annotation('test annotation'),
    1, '$model->attach_annotation() with annotation, but no $model->id_run()' );
is(
    $model->{annotations}->[0],
    'test annotation',
    'annotation appended to annotations array within $model'
);
is( $model->id_user(), undef,
    'id_user not found by model or current run status' );
...

{
    ...
    isnt( $runs_on_batch->[0], undef,
        '$run->runs_on_batch() has found some runs' );
    ...
    my $name = $run->name();
    is( $name, 'IL1_00001',
        'name generated ok from instrument name and id_run' );
    my $run_folder = $run->run_folder();
    is( $run_folder, '070605_IL1_00001',
        'run_folder generated ok from loading date, instrument name and id_run'
    );
    ...
    isa_ok( $current_run_status, 'npg::model::run_status',
        '$run->current_run_status()' );
    is( $run->current_run_status(),
        $current_run_status, 'current_run_status cached' );
    ...
    isa_ok( $run_statuses->[0], 'npg::model::run_status',
        '$run_statuses->[0]' );
    ...
    isa_ok( $run_annotations->[0], 'npg::model::run_annotation',
        '$run_annotations->[0]' );
    ...
    like(
        $EVAL_ERROR,
        qr{No\ annotation\ to\ save},
        '$run->attach_annotation() with no annotation'
    );
    ...
    my $annotation =
      npg::model::annotation->new( { util => $util, id_annotation => 1 } );
    is( $run->attach_annotation($annotation),
        1, 'attach_annotation successful' );
    ...
    is( $run->recent_mirrored_runs(),
        $recent_mirrored_runs, 'recent_mirrored_runs cached' );
    ...
    is_deeply( $recent_running_runs, $test_deeply,
...
#
{
    ...
    ok( $model->create(),
'created run ok - is_paired without id_run_pair, or declared actual_cycle_count'
    );
}

{
    ...
    my $annotation =
      npg::model::run_annotation->new( { util => $util, id_annotation => 1 } );
    $model->{annotations} = [$annotation];
    eval { $model->create(); };
    is( $EVAL_ERROR, q{},
'created run ok - is_paired with id_run_pair and declared actual_cycle_count'
    );
...

{
    ...
    is( $run1->run_pair->id_run(),
        $run3->id_run(), 'run paired with non-cancelled run' );
}
...

{
    ...
    is( $run->is_paired_read(),
        1, 'run 6 is paired read based on tag information' );
}

{
    ...
    is( $run->is_paired_read(),
        0, 'run 7 is paired read based on tag information' );
...

{
    ...
    is( $run->is_paired_read(),
        undef,
        'run 8 is paired read or not, unknown based on tag information' );
...
}
{
    ...
    is( $run->is_paired_read(),
        1, 'run 9 is paired run so it is paired read as well' );
}

lives_ok { $util->fixtures_path(q[t/data/fixtures]); $util->load_fixtures; }
'a new set of fixtures loaded';

{
    ...
    cmp_ok(
        $model->run_folder, 'eq',
        DateTime->now()->strftime(q(%y%m%d)) . '_HS3_09952_A',
        'HiSeq run folder'
    );
}

{
    ...
    cmp_ok(
        $model->run_folder, 'eq',
        DateTime->now()->strftime(q(%y%m%d)) . '_HS3_09953_B_20353ABXX',
        'HiSeq run folder'
    );
}
{
    ...
    'created run ok - folder name and glob passed';
    cmp_ok( $model->folder_name, 'eq', '110401_HS3_B939_B_20353ABXX',
        'folder name set' );
    cmp_ok( $model->run_folder, 'eq', '110401_HS3_B939_B_20353ABXX',
        'run folder overrided' );
    cmp_ok(
        $model->folder_path_glob, 'eq',
        '/{export,nfs}/sf40/ILorHSany_sf40/*/',
        'folder path glob set'
    );
}

{
    ...
    ok( !$m->validate_team('dodo'),
        'team validation failed for non-existing team' );
