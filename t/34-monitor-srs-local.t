...

{
...

    {
        ...
        is( $test->is_run_completed($NOT_RUNFOLDER),
            0, 'Default to \'not complete\' when no information is available' );

    }

    is(
        $test->is_run_completed("$MOCK_STAGING/IL5/incoming/100621_IL5_01204"),
        1, 'Report completed run'
    );

    is(
        $test->is_run_completed("$MOCK_STAGING/IL3/incoming/100622_IL3_01234"),
        0, 'Report not completed run'
    );
...

{
    ...
    is( $test->get_latest_cycle($complete_run_path),
        37, 'Latest cycle derived from \'Intensities\' directory' );
    ...
    is( $test->get_latest_cycle($complete_run_path),
        52, 'Latest cycle derived from StatusUpdate.xml' );
