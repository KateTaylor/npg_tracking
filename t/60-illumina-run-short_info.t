...
Readonly::Scalar my $FOLDER_PATH_PREFIX_GLOB_PATTERN =>
  '/{staging,nfs/sf{5,6,9,10,11,12,13,14,15,16,17}}/{IL,HS}*/*/';
...
has q{_short_path} =>
  ( isa => q{Str}, is => q{ro}, lazy_build => 1, init_arg => undef );

sub _build__short_path {
    my ($self) = @_;
    my @dir =
      $self->has_run_folder()
      ? glob $self->_folder_path_glob_pattern() . $self->run_folder()
      : $self->has_id_run() ? glob $self->_folder_path_glob_pattern()
      . q(*_{r,})
      . $self->id_run() . q{*}
      : $self->has_name()
      ? glob $self->_folder_path_glob_pattern() . q{*} . $self->name()
...
sub create_staging {
    ...
    my $next = $base . q[/staging];
    `mkdir $next`;
    $next = $next . q[/] . $instr_prefix . q[2];
    `mkdir $next`;
    $next = $next . q[/analysis];
    `mkdir $next`;
    ...
    $next = $next . q[/Data];
    `mkdir $next`;
    $next = $next . q[/Intensities];
    `mkdir $next`;

    my $base_calls = $next . q[/BaseCalls];
    `mkdir $base_calls`;
    my $gerald = $next . q[/Bustard-2009-10-01];
    `mkdir $gerald`;
    $gerald = $next . q[/GERALD-2009-10-01];
    `mkdir $gerald`;
...
{
    my $short_info;
    lives_ok { $short_info = test::short_info->new( {} ); }
    q{no error creating no-arg test object};
...
#### test where run_folder is given in the constructor
{
    ...
    is( $short_info->short_reference(),
        $run_folder, q{short_reference returns run_folder first} );
    ...
    is( $short_info->short_reference(),
        $hs_runfolder, q{HS short_reference returns run_folder first} );
    is( $short_info->instrument_string(),
        q{HS2}, q{HS instrument name correctly worked out} );
    is( $short_info->slot(), q{B}, q{HS slot returned ok} );
    is( $short_info->flowcell_id(),
        q{205NNABXX}, q{HS flowcell_id returned ok} );
}
...
# where we can't find out the run_folder
{
    ...
    is( $short_info->short_reference(),
        $id_run, q{short_reference returns id_run} );
    throws_ok { my $name = $short_info->name(); }
    qr{No[ ]paths[ ]to[ ]run[ ]folder[ ]found},
q{no runfolder found with which to return the path, so name/runfolder cannot be worked out};
}
...
# where we can find the run_folder
{
    ...
    is( $short_info->name(), $name, q{name worked out correctly} );
    is( $short_info->short_reference(),
        $run_folder, q{short_reference returns run_folder} );
    ...
    is( $short_info->short_reference(),
        $id_run, q{HS short_reference returns id_run} );
    is( $short_info->name(), q{HS2_1234}, q{HS name worked out correctly} );
    is(
        $short_info->short_reference(),
        q{123456_HS2_1234_B_205NNABXX},
        q{HS short_reference returns run_folder}
    );
}
...
#### test where name is give in the constructor
{
    ...
    is( $short_info->short_reference(), $name,
        q{short_reference returns name} );
    is( $short_info->id_run(), $id_run, q{id_run worked out correctly} );
    is( $short_info->short_reference(),
        $id_run, q{short_reference returns id_run} );
    is( $short_info->run_folder(),
        $run_folder, q{run_folder worked out correctly} );
    is( $short_info->short_reference(),
        $run_folder, q{short_reference returns run_folder} );
}

{
    ...
    is( $short_info->id_run(), 1234,
        q{id_run worked out correctly, no leading zero} );
    is( $short_info->name(), q[IL2_1234],
        q{name does not contain leading zero for id run} );
    is( $short_info->run_folder(), q[123456_IL2_01234], q{run_folder as set} );
    is( $short_info->short_reference(),
        q[123456_IL2_01234], q{short_reference returns run_folder} );
    is( $short_info->instrument_string(),
        q{IL2}, q{IL instrument name correctly worked out} );
    is( $short_info->flowcell_id(),
        q{}, q{IL flowcell_id returns empty string} );
...

{
    ...
    is(
        $short_info->run_folder,
        q(120113_MS1_7362_A_MS0002061-00300),
        q(run folder okay)
    );
