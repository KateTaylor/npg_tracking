...
$util->dbh->do(
    "update instrument set iscurrent=0 where id_instrument_format=11;")
  or die 'Failed to delete MiSeq instruments from the test DB '
  . $util->dbh->errstr;
diag 'To make this test work, MiSeq instruments are set as not current';

my $model = npg::model::instrument_status->new(
    { util => $util, id_instrument_status => 15 } );
#############
...
like( $instruments->{IL9}->[0]->{up}->datetime(),
    qr{2007-09-19}, 'first period on IL9 up' );
isa_ok( $instruments->{IL9}->[0]->{down},
    'DateTime', 'first period on IL9 down' );
is( $instruments->{IL9}->[0]->{down}->datetime(),
    q{2007-09-20T15:02:52}, 'first period on IL9 down' );
is( $instruments->{IL9}->[1]->{up}->datetime(),
    q{2007-10-02T11:02:52}, 'second period on IL9 up' );
like( $instruments->{IL9}->[1]->{down}->datetime(),
    qr{$date_today}, 'second period on IL9 down' );

my $uptime = $model->uptime_for_all_instruments($days);
isa_ok( $uptime, 'ARRAY', '$model->uptime_for_all_instruments()' );
is( $model->uptime_for_all_instruments($days),
    $uptime, '$model->uptime_for_all_instruments() cached ok' );
is( $uptime->[0]->[1],
    '0.00', 'percentage uptime for all instruments, first hour, is correct' );
is( $uptime->[0]->[0]->datetime,
    '2007-09-19T00:00:00', 'time of first is correct' );
is( $uptime->[14]->[1],
    '33.33', 'percentage uptime for all instruments, 14th hour, is correct' );
...
is( $model->average_percentage_uptime_for_day($days),
    $table_rows, '$model->average_percentage_uptime_for_day() cached ok' );
...
isa_ok( $instrument_up_times, 'ARRAY',
    '$model->instrument_percentage_uptimes()' );
is( $model->instrument_percentage_uptimes($days),
    $instrument_up_times, '$model->instrument_percentage_uptimes() cached ok' );
...
is( $instrument_up_times->[0],
    '100.00', 'uptime of last 90 days is correct (default)' );
