...

{
    ...
    my @sorted_run_status_dicts =
      sort { $a->{temporal_index} <=> $b->{temporal_index} } @{$rsds};

    cmp_deeply( $rsds, \@sorted_run_status_dicts,
        'Status list is in temporal index order' );
}
...

{
    ...
    is( $count_runs, 2,
        'unprimed cache count_runs - analysis complete - all formats' );
    is( $count_runs, 2,
        'primed cache count_runs - analysis complete - all formats' );
}
{
    ...
    is( $count_runs, 3,
'unprimed cache count_runs(3) - run complete - id_instrument_format = 10'
    );
    is( $count_runs, 3,
        'primed cache count_runs(3) - run complete - id_instrument_format = 10'
    );
    ...
    is( $count_runs, 1,
        'unprimed cache count_runs(1) - run complete - id_instrument_format = 7'
    );
    is( $count_runs, 1,
        'primed cache count_runs(1) - run complete - id_instrument_format = 7'
    );
