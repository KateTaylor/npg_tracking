...
my @samples_6551_1 =
  qw/phiX_for_spiked_buffers SS109114 SS109305 SS117077 SS117886 SS127358 SS127858 SS128220 SS128716 SS129050 SS129764 SS130327 SS131636/;
my @studies_6551_1 =
  ( 'Illumina Controls', 'Discovery of sequence diversity in Shigella sp.' );

{
    ...
    is( $lims->spiked_phix_tag_index, undef,
        'spiked phix tag index undefined' );
    is( scalar $lims->descendants, 185, '185 descendant lims' );
    is( scalar $lims->children,    8,   '8 child lims' );
    is(
        $lims->to_string,
        'st::api::lims object, driver - xml, batch_id 12141, id_run 6551',
        'object as string'
    );
    throws_ok { $lims->_list_of_properties( q[id], q[bird] ) }
    qr/Invalid object type bird in st::api::lims::_list_of_properties/,
      'error with invalid entity name';
    throws_ok { $lims->_list_of_properties( q[colour], q[sample] ) }
    qr/Invalid property colour in st::api::lims::_list_of_properties/,
      'error with invalid entity name';
    is( scalar( $lims->library_names ),
        0, 'batch-level library_names list empty' );
    is( scalar( $lims->sample_names ),
        0, 'batch-level sample_names list empty' );
...
'st::api::lims object, driver - xml, batch_id 12141, id_run 6551, position 1',
    ...
    is(
        join( q[,], $lims->library_names ),
        join( q[,], sort @libs_6551_1 ),
        'pool lane-level library_names list'
    );
    is(
        join( q[,], $lims->sample_names ),
        join( q[,], sort @samples_6551_1 ),
        'pool lane-level sample_names list'
    );
    is(
        join( q[,], $lims->study_names ),
        join( q[,], sort @studies_6551_1 ),
        'pool lane-level study_names list'
    );

    my $with_spiked_phix = 1;
    is(
        join( q[,], $lims->library_names($with_spiked_phix) ),
        join( q[,], sort @libs_6551_1 ),
        'pool lane-level library_names list $with_spiked_phix = 1'
    );
    is(
        join( q[,], $lims->sample_names($with_spiked_phix) ),
        join( q[,], sort @samples_6551_1 ),
        'pool lane-level sample_names list $with_spiked_phix = 1'
    );
    is(
        join( q[,], $lims->study_names($with_spiked_phix) ),
        join( q[,], sort @studies_6551_1 ),
        'pool lane-level study_names list $with_spiked_phix = 1'
    );

    is(
        join( q[,], $lims->library_ids($with_spiked_phix) ),
'2389196,2798523,2798524,2798525,2798526,2798527,2798528,2798529,2798530,2798531,2798532,2798533,2798534',
    ...
    is(
        join( q[,], $lims->sample_ids($with_spiked_phix) ),
'1093818,1093819,1093820,1093821,1093822,1093823,1093824,1093825,1093826,1093827,1093828,1093829,1255141',
    ...
    is( join( q[,], $lims->study_ids($with_spiked_phix) ),
        '198,297', 'pool lane-level study_ids list $with_spiked_phix = 1' );
    is( join( q[,], $lims->project_ids($with_spiked_phix) ),
        '297', 'pool lane-level project_ids list $with_spiked_phix = 1' );
    ...
    is(
        join( q[,], $lims->library_names($with_spiked_phix) ),
        join( q[,], sort @libs ),
        'pool lane-level library_names list $with_spiked_phix = 0;'
    );
    is(
        join( q[,], $lims->sample_names($with_spiked_phix) ),
        join( q[,], sort @samples ),
        'pool lane-level sample_names list $with_spiked_phix = 0;'
    );
    is(
        join( q[,], $lims->study_names($with_spiked_phix) ),
        join( q[,], sort @studies ),
        'pool lane-level study_names list $with_spiked_phix = 0;'
    );
    is( join( q[,], $lims->library_types() ),
        'Standard', 'pool lane-level library types list' );
...
{
    my $lims =
      st::api::lims->new( id_run => 6551, position => 1, tag_index => 0 );
    is( $lims->lane_id(), undef,
        q{lane_id undef for id_run 6551, position 1, tag_index defined} );
    is( $lims->batch_id,     12141,    'batch id is 12141 for lane 1' );
    is( $lims->is_control,   0,        'lane 1 is not control' );
    is( $lims->is_pool,      1,        'lane 1 is pool' );
    is( $lims->library_id,   2988920,  'lib id' );
    is( $lims->library_name, '297p11', 'pool lib name' );
    is( $lims->contains_nonconsented_human,
        0, 'does not contain nonconsented human' );
    is( $lims->contains_unconsented_human,
        0, 'does not contain unconsented human (back compat)' );
    is( $lims->contains_nonconsented_xahuman,
        0, 'does not contain nonconsented X and autosomal human' );
    is( $lims->tag_sequence,          undef, 'tag_sequence undefined' );
    is( scalar keys %{ $lims->tags }, 13,    '13 tags defined' );
    ...
    is(
        $lims->to_string,
'st::api::lims object, driver - xml, batch_id 12141, id_run 6551, position 1, tag_index 0',
        'object as string'
    );
    is(
        join( q[,], $lims->library_names ),
        join( q[,], sort @libs_6551_1 ),
        'tag 0 library_names list'
    );
    is(
        join( q[,], $lims->sample_names ),
        join( q[,], sort @samples_6551_1 ),
        'tag 0 sample_names list'
    );
    is(
        join( q[,], $lims->study_names ),
        join( q[,], sort @studies_6551_1 ),
        'tag 0 study_names list'
    );
    ...
    is(
        join( q[,], $lims->library_names($with_spiked_phix) ),
        join( q[,], sort @libs_6551_1 ),
        'tag 0 library_names list $with_spiked_phix = 1'
    );
    is(
        join( q[,], $lims->sample_names($with_spiked_phix) ),
        join( q[,], sort @samples_6551_1 ),
        'tag 0 sample_names list $with_spiked_phix = 1'
    );
    is(
        join( q[,], $lims->study_names($with_spiked_phix) ),
        join( q[,], sort @studies_6551_1 ),
        'tag 0 study_names list $with_spiked_phix = 1'
    );
    ...
    is(
        join( q[,], $lims->library_names($with_spiked_phix) ),
        join( q[,], sort @libs ),
        'tag 0 library_names list $with_spiked_phix = 0;'
    );
    is(
        join( q[,], $lims->sample_names($with_spiked_phix) ),
        join( q[,], sort @samples ),
        'tag 0 sample_names list $with_spiked_phix = 0;'
    );
    is(
        join( q[,], $lims->study_names($with_spiked_phix) ),
        join( q[,], sort @studies ),
        'tag 0 study_names list $with_spiked_phix = 0;'
    );
}

{
    my $lims =
      st::api::lims->new( id_run => 6551, position => 1, tag_index => 168 );
    is( $lims->is_control, 1, 'tag 1/168 is control' );
    is( $lims->is_pool,    0, 'tag 1/168 is not a pool' );
    is( $lims->contains_nonconsented_human,
        0, 'does not contain nonconcented human' );
    is( $lims->contains_nonconsented_xahuman,
        0, 'does not contain nonconsented X and autosomal human' );
    my $lib_id = 2389196;
    ...
    is( join( q[ ], $lims->library_names ),
        $libs_6551_1[0], 'tag 168 library_names list' );
    is( join( q[ ], $lims->sample_names ),
        $samples_6551_1[0], 'tag 168 sample_names list' );
    is( join( q[ ], $lims->study_names ),
        $studies_6551_1[0], 'tag 168 study_names list' );

    my $with_spiked_phix = 0;
    is( join( q[ ], $lims->library_names($with_spiked_phix) ),
        $libs_6551_1[0], 'tag 168 library_names list' );
    is( join( q[ ], $lims->sample_names($with_spiked_phix) ),
        $samples_6551_1[0], 'tag 168 sample_names list' );
    is( join( q[ ], $lims->study_names($with_spiked_phix) ),
        $studies_6551_1[0], 'tag 168 study_names list' );
}

{
    my $lims =
      st::api::lims->new( id_run => 6551, position => 1, tag_index => 1 );
    ...
    is(
        join( q[ ], $lims->library_names ),
        'SS109305 2798523',
        'tag 1 library_names list'
    );
    is( join( q[ ], $lims->sample_names ),
        'SS109305', 'tag 1 sample_names list' );
    is( join( q[ ], $lims->study_names ),
        $studies_6551_1[1], 'tag 1 study_names list' );

    my $with_spiked_phix = 0;
    is(
        join( q[ ], $lims->library_names($with_spiked_phix) ),
        'SS109305 2798523',
        'tag 1 library_names list $with_spiked_phix = 0'
    );
    is( join( q[ ], $lims->sample_names($with_spiked_phix) ),
        'SS109305', 'tag 1 sample_names list $with_spiked_phix = 0' );
    is( join( q[ ], $lims->study_names($with_spiked_phix) ),
        $studies_6551_1[1], 'tag 1 study_names list $with_spiked_phix = 0' );
}

{
    my $lims =
      st::api::lims->new( id_run => 6607, position => 1 );    # non-pool lane
    ...
    foreach my $method (@methods) {
        lives_ok { $lims->$method }
        qq[invoking method or attribute $method does not throw an error];
    }

    is(
        join( q[ ], $lims->library_names ),
        'BS_3hrsomuleSm_202790 3033734',
        'non-pool lane library_names list'
    );
    is(
        join( q[ ], $lims->sample_names ),
        'BS_3hrsomuleSm_202790',
        'non-pool lane sample_names list'
    );
    is(
        join( q[ ], $lims->study_names ),
        'Schistosoma mansoni methylome',
        'non-pool lane study_names list'
    );
}

{
    my $lims =
      st::api::lims->new( id_run => 6607, position => 2 );    # non-pool lane
    ...
    throws_ok { $lims->seq_qc_state }
    qr/Unexpected value 'some' for seq qc state/,
...

{
    ...
    is( $lims->study_name, '3C and HiC of Plasmodium falciparum IT',
        'study name' );
    my $description =
q[Illumina sequencing of chromatin conformation capture and its derivatives is being carried out to study nuclear architecture in antigenically selected lines of Plasmodium falciparum. This data is part of a pre-publication release. For information on the proper use of pre-publication data shared by the Wellcome Trust Sanger Institute (including details of any publication moratoria), please see http://www.sanger.ac.uk/datasharing/];
    ...
    is( $lims->project_name, '3C and HiC of Plasmodium falciparum IT',
        'project name' );
    my $expected_tags = {
        1 => 'ATCACGTT',
        2 => 'CGATGTTT',
    ...
    is(
        $lims->to_string,
'st::api::lims object, driver - xml, batch_id 12378, id_run 6607, position 5',
        'object as string'
    );

    my $libs =
'HIC_M_B15C2 3111683,HiC_H_Dd2 3111680,HiC_H_LT3D7 3111687,HiC_H_OFF_DCJ 3111682,HiC_H_ON_DCJ 3111679,HiC_H_PQP1 3111681,HiC_M_3D7 3111685,HiC_M_ER3D7 3111686,HiC_M_Rev1 3111684';
    my $samples =
'HIC_M_B15C2,HiC_H_Dd2,HiC_H_LT3D7,HiC_H_OFF_DCJ,HiC_H_ON_DCJ,HiC_H_PQP1,HiC_M_3D7,HiC_M_ER3D7,HiC_M_Rev1';
    my $study = '3C and HiC of Plasmodium falciparum IT';
    is( join( q[,], $lims->library_names ),
        $libs, 'pooled not-spiked lane library_names list' );
    is( join( q[,], $lims->sample_names ),
        $samples, 'pooled not-spiked lane sample_names list' );
    is( join( q[,], $lims->study_names ),
        $study, 'pooled not-spiked lane study_names list' );

    my $with_spiked_phix = 0;
    is( join( q[,], $lims->library_names($with_spiked_phix) ),
        $libs,
        'pooled not-spiked lane library_names list $with_spiked_phix = 0' );
    is( join( q[,], $lims->sample_names($with_spiked_phix) ),
        $samples,
        'pooled not-spiked lane sample_names list $with_spiked_phix = 0' );
    is( join( q[,], $lims->study_names($with_spiked_phix) ),
        $study,
        'pooled not-spiked lane study_names list $with_spiked_phix = 0' );
}

{
    my $lims =
      st::api::lims->new( id_run => 6607, position => 5, tag_index => 1 );
    my $lims_nofb =
      st::api::lims->new( id_run => 6607, position => 5, tag_index => 1 );
...

{
    ...
    ok( !$lims->is_control,
'lane is not a control (despite having a control tag within its hyb buffer tag)'
    );
}
...

{
    ...
    is( $lims->contains_unconsented_human,
        1, 'contains unconsented human (back compat)' );
    ...
    $lims =
      st::api::lims->new( batch_id => 13861, position => 2, tag_index => 0 );
    ok( $lims->contains_nonconsented_human,
        'tag 0 contains unconsented human' );
}

{
    ...
    $lims =
      st::api::lims->new( batch_id => 16442, position => 1, tag_index => 2 );
    is( $lims->bait_name, 'Human all exon 50MB', 'bait name for a plex' );
    $lims =
      st::api::lims->new( batch_id => 16442, position => 1, tag_index => 3 );
    is( $lims->bait_name, 'Fox bait', 'bait name for another plex' );
    $lims =
      st::api::lims->new( batch_id => 16442, position => 8, tag_index => 5 );
    is( $lims->bait_name, 'Mouse some exon', 'bait name for yet another plex' );
    $lims =
      st::api::lims->new( batch_id => 16442, position => 1, tag_index => 4 );
    is( $lims->bait_name, undef, 'bait name undefined if no bait element' );
    $lims =
      st::api::lims->new( batch_id => 16442, position => 1, tag_index => 5 );
    is( $lims->bait_name, undef,
        'bait name undefined if no bait name tag is empty' );
    $lims =
      st::api::lims->new( batch_id => 16442, position => 1, tag_index => 168 );
    ...
    is( $lims->bait_name, undef,
'bait name undefined for a control lane despite the presence of the bait element'
    );
}
...
'run does contain nonconsented X and autosomal human does not propagate to run level'
    ...
    is( $lims->contains_nonconsented_xahuman,
        0, 'lane 2 does not contain nonconsented X and autosomal human' );
    $lims = st::api::lims->new( id_run => 8260, position => 8 );
    is( $lims->contains_nonconsented_xahuman,
        1, 'lane 8 does contain nonconsented X and autosomal human' );
    $lims =
      st::api::lims->new( id_run => 8260, position => 2, tag_index => 33 );
    is( $lims->contains_nonconsented_xahuman,
        0,
        'plex 33 lane 2 does not contain nonconsented X and autosomal human' );
    $lims =
      st::api::lims->new( id_run => 8260, position => 8, tag_index => 57 );
    is( $lims->contains_nonconsented_xahuman,
        1, 'plex 57 lane 8 does contain nonconsented X and autosomal human' );
}
...
#diag q[Checking tag created by _build_tag is equal to the description value, not the tag value in expected_sequence, if a tag is available in the description];
    ...
    cmp_ok( $tags[0]->{5}, q(ne), q(ACAGTGGT),
        'Do not use expected_sequence sequence for tag' );
    cmp_ok( $tags[0]->{5}, q(eq), q(GTAGAC),
        'Use sample_description sequence for tag' );
...
#diag q[Checking tag created by _build_tag is equal to the expected_sequence where no sample description is available];
    ...
    cmp_ok( $tags1[0]->{144},
        q(eq), q(CCTGAGCA), 'Use expected_sequence sequence for tag' );

#diag q[Checking library_type is changed to 3 prime poly-A pulldown where tag is taken from sample description];
    my $lims85 =
      st::api::lims->new( batch_id => 19158, position => 8, tag_index => 5 );
    is( $lims85->library_type, '3 prime poly-A pulldown', 'library type' );
    is( $lims85->tag_sequence, 'GTAGAC',
        'plex tag sequence from sample description' );
    ok( $lims85->sample_description =~ /end enriched mRNA/sm,
        'sample description available' );

#diag q[Checking library_type is not changed to 3 prime poly-A pulldown where no sample description is available];
    my $lims1144 =
      st::api::lims->new( batch_id => 19158, position => 1, tag_index => 144 );
    isnt( $lims1144->library_type, '3 prime poly-A pulldown', 'library type' );
    is( $lims1144->tag_sequence, 'CCTGAGCA',
        'plex tag sequence directly from batch xml' );
}

{
    ...
    my $lims =
      st::api::lims->new( batch_id => 17763, position => 1, tag_index => 1 );
    is( $lims->study_title(), 'hifi test',      q{study title} );
    is( $lims->study_name(),  'Kapa HiFi test', 'study name' );
    is( $lims->study_accession_number(), undef,
        q{no study accession obtained} );
    is( $lims->study_publishable_name(),
        q{hifi test}, q{study title returned as publishable name} );

    $lims =
      st::api::lims->new( batch_id => 17763, position => 1, tag_index => 2 );
    ok( !$lims->alignments_in_bam,
        'no alignments in BAM when false in corresponding XML in study' );
    is(
        $lims->study_title(),
        'Genetic variation in Kuusamo',
        q{study title obtained}
    );
    is( $lims->study_accession_number(),
        'EGAS00001000020', q{study accession obtained} );
    is( $lims->study_publishable_name(),
        'EGAS00001000020', q{accession returned as study publishable name} );
    is( $lims->sample_publishable_name(),
        q{ERS003242}, q{sample publishable name returns accession} );
    ok(
        !$lims->separate_y_chromosome_data,
        'do not separate y chromosome data'
    );

    $lims =
      st::api::lims->new( batch_id => 22061, position => 1, tag_index => 66 );
    ok( $lims->separate_y_chromosome_data, 'separate y chromosome data' );
}

{
    my $sample_description =
'AB GO (grandmother) of the MGH meiotic cross. The same DNA was split into three aliquots (of which this';
    is(
        st::api::lims::_tag_sequence_from_sample_description(
            $sample_description),
        undef,
q{tag undefined for a description containing characters in round brackets}
    );
    $sample_description =
"3' end enriched mRNA from morphologically abnormal embryos from dag1 knockout incross 3. A 6 base indexing sequence (GTAGAC) is bases 5 to 11 of read 1 followed by polyT.  More information describing the mutant phenotype can be found at the Wellcome Trust Sanger Institute Zebrafish Mutation Project website http://www.sanger.ac.uk/cgi-bin/Projects/D_rerio/zmp/search.pl?q=zmp_phD";
    is(
        st::api::lims::_tag_sequence_from_sample_description(
            $sample_description),
        q{GTAGAC},
        q{correct tag from a complex description}
    );
    ...
    is(
        st::api::lims::_tag_sequence_from_sample_description(
            $sample_description),
        undef,
        q{tag undefined for a description with carriage return}
    );
}
...

{
    ...
    my $ss = st::api::lims->new(
        id_run      => 10262,
        path        => $path,
        driver_type => 'samplesheet'
    );
    isa_ok( $ss->driver, 'st::api::lims::samplesheet',
        'samplesheet driver object instantiated' );
    my @lanes;
    ...
    is( $lanes[0]->id_run, 10000,
        'lane id_run as set, differs from Experiment Name' );
...
'can get lane-level objects, get warning about setting id_run from Experiment Name';
    ...
    is( $lane->id_run, 10262,
        'lane id_run set correctly from Experiment Name' );
    ...
    is( $plexes[0]->id_run, 10262,
        'id_run of the first plexe set correctly from Experiment Name' );
    is( $plexes[0]->library_id,  7583411, 'library_id of the first plex' );
    is( $plexes[0]->sample_name, 'LIA_1', 'sample_name of the first plex' );
    is( $plexes[0]->sample_id,
        undef, 'sample_id of the first plex in undefined' );
    is( $plexes[0]->is_pool,    0,     'is_pool false on plex level' );
    is( $plexes[0]->is_control, undef, 'is_control false on for a plex' );
    is( $plexes[0]->default_tag_sequence,
        'ATCACGTT', 'default tag sequence of the first plex' );
    is( $plexes[0]->tag_sequence, 'ATCACGTT',
        'tag sequence of the first plex' );
    is( $plexes[95]->position,  1,  'position of the last plex is 1' );
    is( $plexes[95]->tag_index, 96, 'tag_index of the last plex is 96' );
    is( $plexes[95]->id_run, 10262,
        'id_run of the last plex set correctly from Experiment Name' );
    is( $plexes[95]->tag_sequence, 'GTCTTGGC',
        'tag sequence of the last plex' );
...

{
    ...
    qr/Attribute \(path\) is required/,
      'error invoking a driver method on an object with path undefined';
    ...
    qr/Validation failed for 'NpgTrackingReadableFile'/,
      'error invoking a driver method on an object with non-existing path';
...
'error invoking a driver method on an object for a non-existing tag_index';
    ...
    is( $ss->tag_sequence, $ss->default_tag_sequence,
