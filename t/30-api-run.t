...

{
    ...
    throws_ok { $run->is_paired_read(); }
    qr{No data on paired/single read available yet},
      'no tag information available for run 2';
    throws_ok { $run->is_single_read(); }
    qr{No data on paired/single read available yet},
      'no tag information available for run 2 when call is_single_read';
}
...

{
    ...
    is( $run->run_pair()->batch_id(),
        $run->batch_id(), 'batch id the same within run pair' );
    ...
    my $lims;
    isa_ok( $lims = $run->lims(),
        'st::api::lims', 'lims accessor returns st::api::lims object' );
    ...
    is( $lims->batch_id, 1001,
        'run_pair lims object batch id is set correctly' );
...
local $ENV{NPG_WEBSERVICE_CACHE_DIR} = q[];
{
    ...
    my $runs;
    lives_ok { $runs = $run->recent_running_runs(); }
    'no croak on recent_running_runs';
...

{
    ...
    lives_ok { $run->add_tags( 'rta', 'paired_read', 'staging' ); }
    'no croak when adding new tags rta, paired_read and staging';
}

{
    ...
    lives_ok { $run->remove_tags('staging'); }
    'no croak when removing rta and staging';
}
