...

{
    ...
    is( ( scalar @{$runs} ),
        2, 'limited, id_rsd-restricted runs for instrument' );
...

{
    ...
    is( ( scalar @{$runs} ),
        2, 'limited, offset, id_rsd-restricted runs for instrument' );
    is( $runs->[0]->id_run(), 12, 'first run ok' );
...

{
    ...
    isa_ok( $model, 'npg::model::instrument',
        '$model with name and id_instrument predeclared' );
...

{
    ...
    is( $model->current_run(), $current_run,
        '$model->current_run() cached ok' );
    is( $model->model(), 'HK', '$model->model() retrieved correctly' );
    is( $model->id_manufacturer(),
        10, '$model->id_manufacturer() retrieved correctly' );
    isa_ok( $model->manufacturer(),
        'npg::model::manufacturer', '$model->id_manufacturer()' );
    ...
    isa_ok(
        ...
        'model->instrument_statuses()->[0]'
    );

    my $instrument_mods = $model->instrument_mods();
    isa_ok( $instrument_mods, 'ARRAY', '$model->instrument_mods()' );
    is( $model->instrument_mods(),
        $instrument_mods, '$model->instrument_mods() cached ok' );
    isa_ok( $instrument_mods->[0], 'npg::model::instrument_mod',
        '$model->instrument_mods()->[0]' );

    my $current_instrument_mods = $model->current_instrument_mods();
    isa_ok( $current_instrument_mods, 'HASH',
        '$model->current_instrument_mods()' );
    is( $model->current_instrument_mods(),
        $current_instrument_mods,
        '$model->current_instrument_mods() cached ok' );
    is( keys %{$current_instrument_mods},
        1, '1 key found for $model->current_instrument_mods()' );

    my $current_instrument_status = $model->current_instrument_status();
    isa_ok(
        $current_instrument_status,
        'npg::model::instrument_status',
        q{$model->current_instrument_status()}
    );
    ...
    is(
        $model->current_instrument_status->instrument_status_dict->description,
        'up', 'current status is up'
    );
    ...
    cmp_deeply( $model->possible_next_statuses,
        $expected_next, 'expected next statuses' );

    my $run_status  = 'run complete';
    my $auto_status = 'wash required';
    is(
        $model->status_to_change_to($run_status),
        'wash required',
qq["up" should be changed to "$auto_status", "$run_status" run status given]
    );
    is(
        $model->current_instrument_status->instrument_status_dict->description,
        'up', 'current status is still up'
    );
    ok( !$model->status_to_change_to(),
        'no need to change "up" status, no run status given' );
    ...
    is(
        $model->current_instrument_status->instrument_status_dict->description,
        $auto_status, qq[status changed automatically to "$auto_status"]
    );
}

{
    ...
    is( $instr->staging_dir(), '/staging/IL29/incoming',
        'staging directory correct' );
    ...
    cmp_bag(
        \@desigs,
        [ 'Hot spare', 'R&D' ],
        'deal with multiple designations'
    );
}

{
    ...
    ok( !$model->status_to_change_to(),
        'no need to auto change status, as status is down' );
}

{
    ...
    is(
        $model->current_instrument_status->instrument_status_dict()
          ->description(),
        'up', 'cbot current status is up'
    );
    ok( !$model->status_to_change_to(),
        'no need for an automatic status change' );
    lives_ok { $model->status_reset('wash required') }
    'no error changing status';
    is(
        $model->current_instrument_status->instrument_status_dict()
          ->description(),
        'wash required',
        'cbot current status is wash required'
    );
...
#throws_ok {$model->status_reset('wash performed')} qr/cBot1 \"wash performed\" status cannot follow current \"wash required\" status/, 'error moving from "wash required" directly to "wash performed"';
    lives_ok { $model->status_reset('wash performed') }
    'no error moving from "wash required" directly to "wash performed"';
    lives_ok { $model->status_reset('wash in progress') }
    'no error changing status';
    lives_ok { $model->status_reset('wash performed') }
    'no error changing status';
    is(
...
lives_ok { $util->fixtures_path(q[t/data/fixtures]); $util->load_fixtures; }
'a fresh set of fixtures loaded';
{
    my $model =
      npg::model::instrument->new( { util => $util, id_instrument => 48, } );
...
{
    my $model =
      npg::model::instrument->new( { util => $util, id_instrument => 35, } );
...

{
    ...
    is( $model->fc_slots2current_runs,
        undef, 'does not have mapping of slots to current runs' );
    is( $model->fc_slots2blocking_runs,
        undef, 'does not have mapping of slots to blocking runs' )
}

{
    my $model =
      npg::model::instrument->new( { util => $util, id_instrument => 34, } );
    ...
    ok( !$model->status_to_change_to('analysis in progress'),
        'no status to change to' );
    ok( !$model->autochange_status_if_needed('analysis in progress'),
        'no autochange status' );
    throws_ok { $model->status_reset() }
    qr/Status to change to should be defined/,
      'error when status to set to not given';
}

{
    ...
    cmp_deeply( $model->fc_slots2current_runs,
        $expected, 'empty mapping of slots to current runs' );
    cmp_deeply( $model->fc_slots2blocking_runs,
        $expected, 'empty mapping of slots to blocking runs' );
    ok( !$model->is_idle, 'instrument is not idle' );
    ok( !$model->status_to_change_to('analysis in progress'),
        'no status to change to' );
    ok( !$model->autochange_status_if_needed('analysis in progress'),
...
{
    my $model =
      npg::model::instrument->new( { util => $util, id_instrument => 36, } );
    ok( $model->is_hiseq_instrument, 'is hiseq instrument' );
    ...
    cmp_deeply( $model->fc_slots2current_runs,
        $expected, 'non-empty mapping of slotA to runs' );
    ok( !$model->is_idle,             'instrument is not idle' );
    ok( !$model->status_to_change_to, 'no status to change to' );
}

{
    ...
    cmp_deeply( $model->fc_slots2current_runs,
        $expected, 'non-empty mapping of both slots to current runs' );
    cmp_deeply(
        $model->fc_slots2blocking_runs,
        { fc_slotA => [], fc_slotB => [], },
        'empty mapping of both slots to blocking runs'
    );
}

{
    ...
    @{ $fc_slots2current_runs->{fc_slotA} } =
      sort @{ $fc_slots2current_runs->{fc_slotA} };

    cmp_deeply( $fc_slots2current_runs, $expected,
        'non-empty mapping of both slots to current runs' );
    cmp_deeply(
        $model->fc_slots2blocking_runs,
        { fc_slotA => [], fc_slotB => [], },
        'empty mapping of both slots to blocking runs'
...

{
    ...
    ok( !$model->current_run_by_id(22),
        'undef returned for non-existing current run' );
    $run = $model->current_run_by_id(9950);
    is( ref $run, 'npg::model::run',
        'run object returned for an existing current run' );
...

{
    my $dbh = $util->dbh;
    my $update =
q[update run_status set id_run_status_dict=2 where id_run=9951 and iscurrent=1];
    ...
    cmp_deeply(
        $model->fc_slots2blocking_runs,
        { fc_slotA => [9951], fc_slotB => [9951], },
        'correct mapping of slots to blocking runs'
    );
}

{
    ...
    my $next_up =
q[planned service,planned repair,down for repair,wash required,wash in progress];
    is(
        join( q[,],
            @{ npg::model::instrument::possible_next_statuses4status('up') } ),
        $next_up,
        'possible_next_statuses for "up" called as module function'
    );
    is( join( q[,], @{ $model->possible_next_statuses4status('up') } ),
        $next_up,
        'possible_next_statuses for "up" called as an object method' );
    ...
    throws_ok {
        npg::model::instrument::possible_next_statuses4status('some status');
    }
    ...
    is(
        join( q[,],
            @{ $model->possible_next_statuses4status('wash in progress') } ),
