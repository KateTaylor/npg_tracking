...
lives_ok {
    $test = $schema->resultset('Run')->find( { id_run => $test_run_id } );
}
'Create test object';
...

{
    ...
    isa_ok( $tag_rs, 'npg_tracking::Schema::Result::Tag', 'Tag result set' );

    isa_ok( $user_rs, 'npg_tracking::Schema::Result::User', 'User result set' );
}

# Conventional tests for current_run_status method
{
    is( $test->current_run_status_description(),
        'run complete', 'Current run status returned' );
}
...
# Status updates.
{
    ...
    isnt( $run_status_rs->first->date(),
        $test_date, 'New entry is an update not a copy' );
...
my $paired_read_query = {
    id_run => $test_run_id,
    id_tag => 17
};

my $single_read_query = {
    id_run => $test_run_id,
    id_tag => 18
};
...

{
    ...
    lives_ok { $test->unset_tag( 3, 'rta' ) } 'Use general method to unset tag';
...
# Tests for updating instrument status when it is 'planned maintenance'.
{
    ...
    foreach my $run ( ( $one_of_two_active, $active_one_of_two, $single ) ) {
        ...
        lives_ok {
            $instrument->update_instrument_status( 'planned repair',
                'pipeline' );
        }
    ...
    lives_ok {
        $one_of_two_active->update_run_status( 'run cancelled', 'pipeline' );
    }
    '  Run cancelled on a HiSeq with another active run';

    is(
        $one_of_two_active->instrument->current_instrument_status(),
        'planned repair',
        '  Instrument status is not changed'
    );

    lives_ok {
        $active_one_of_two->update_run_status( 'run cancelled', 'pipeline' );
    }
    '  Run cancelled on a HiSeq with no other active run';

    is(
        $active_one_of_two->instrument->current_instrument_status(),
        'down for repair',
        '  Instrument status is now \'down for repair\''
    );
    ...
    is(
        $single->instrument->current_instrument_status(),
        'down for repair',
        '  Instrument status is now \'down for repair\''
    );
    ...
    lives_ok {
        ...
        $single->instrument->update_instrument_status( 'planned repair',
            'pipeline' );
    }
    'Reset instrument statuses';

    lives_ok {
        $one_of_two_active->update_run_status( 'run complete', 'pipeline' );
    }
    ...
    lives_ok {
        $active_one_of_two->update_run_status( 'run complete', 'pipeline' );
    }
    ...
    lives_ok {
        $active_one_of_two->update_run_status( 'run mirrored', 'pipeline' );
    }
    ...
    is(
        $single->instrument->current_instrument_status(),
        'planned repair',
        '  Instrument status has not changed'
    );
    ...
    is(
        $single->instrument->current_instrument_status(),
        'down for repair',
        '  Instrument status is now \'down for service\''
    );

    lives_ok {
        $active_one_of_two->instrument->update_instrument_status(
            'planned service', 'pipeline' );
        $single->instrument->update_instrument_status( 'planned service',
            'pipeline' );
    }
    'Reset instrument statuses';

    lives_ok {
        $one_of_two_active->update_run_status( 'analysis pending', 'pipeline' );
    }
    ...
    lives_ok {
        $active_one_of_two->update_run_status( 'analysis pending', 'pipeline' );
    }
    ...
    is(
        $active_one_of_two->instrument->current_instrument_status(),
        'down for service',
        '  Instrument status changed to down for service'
    );

    lives_ok {
        $one_of_two_active->instrument->update_instrument_status( 'up',
            'pipeline' );
        $active_one_of_two->instrument->update_instrument_status( 'up',
            'pipeline' );
    ...
    is(
        $active_one_of_two->instrument->current_instrument_status(),
        'wash required',
        '  Instrument status changed to wash required'
    );
    ...
    is(
        $single->instrument->current_instrument_status(),
        'wash required',
        '  Instrument status changed to wasg required'
    );
}
...
{    #check single read run
    ...
    lives_and {
        cmp_ok( $r->forward_read->expected_cycle_count,
            '==', 54, ' expected cycle count' );
    }
    'forward read';
...
{    #check paired read run
    ...
    lives_and {
        cmp_ok( $r->forward_read->expected_cycle_count,
            '==', 100, ' expected cycle count' );
    }
    'forward read';
    lives_and {
        cmp_ok( $r->reverse_read->expected_cycle_count,
            '==', 100, ' expected cycle count' );
    }
    'reverse read';
}
...
# Find current status - alter count
{
    ...
    diag $r->run_statuses->search( { iscurrent => 1 } )->count()
      . " current run_statuses for run $test_run_id";
    is( $r->current_run_status_description(),
        undef, 'Return undef for no current run status' );

  TODO: {
        local $TODO =
'hope to get this working with a suitable database schema/constraint and relationship';
        $r->run_statuses->update( { iscurrent => 1 } );
        diag $r->run_statuses->search( { iscurrent => 1 } )->count()
          . " current run_statuses for run $test_run_id";
        dies_ok { $r->current_run_status_description() }
        'Dies for multiple current run statuses';
    }
}
