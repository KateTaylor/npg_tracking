...

{
    ...
    lives_ok {
        $fb =
          Moose::Meta::Class->create_anon_class(
            roles => [qw/npg_tracking::data::bait::find/] )->new_object(
            {
                repository => $repos,
    ...
    is(
        $fb->bait_path,
        "$baits_repos/Human_all_exon_50MB/1000Genomes_hs37d5",
        'bait path bipassing lims object'
    );
}
...
{    # This id_run, position, tag_index should return valid results
    my $test = npg_tracking::data::bait->new(
        id_run     => 7753,
        position   => 1,
        tag_index  => 2,
        repository => $repos
    ...
    'bait path found';
    is(
        $test->bait_intervals_path,
"$baits_repos/Human_all_exon_50MB/1000Genomes_hs37d5/S02972011-GRCh37_hs37d5-CTR.interval_list",
        'bait CTR file found'
    );
    is(
        $test->target_intervals_path,
"$baits_repos/Human_all_exon_50MB/1000Genomes_hs37d5/S02972011-GRCh37_hs37d5-PTR.interval_list",
        'bait PTR file found'
    );

    $test = npg_tracking::data::bait->new(
        id_run     => 7753,
        position   => 1,
        tag_index  => 3,
        repository => $repos
