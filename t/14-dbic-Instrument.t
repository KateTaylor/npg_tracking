...
{
    lives_ok {
        $test = $schema->resultset('Instrument')
          ->find( { id_instrument => $test_instrument_id } );
    }
    'Create test object';

    isa_ok( $test, 'npg_tracking::Schema::Result::Instrument',
        'Correct class' );
    is(
        $test->current_instrument_status(),
        'wash required',
    ...
    $schema->resultset('InstrumentStatus')->create(
        {
            ...
            date                      => DateTime->now()->subtract(
                DateTime::Duration->new( days => $wash_interval + 4 )
            ),
        }
    );
    throws_ok {
        $test->update_instrument_status( 'down', 'joe_engineer',
            'instrument is down' );
    }
...
{
    lives_ok {
        $test->update_instrument_status( 'planned repair', 'joe_loader' );
    }
    ...
    is(
        $instrument_status_rs->first->comment(),
        'instrument is down',
        'original comment'
    );

    lives_ok {
        $test->update_instrument_status( 'request approval',
            'pipeline', 'to approve' );
    }
    'Set a new status';
    ...
    is(
        $instrument_status->comment(),
        'automatic status update : to approve',
        'pipeline comment'
    );
    is(
        $instrument_status->instrument_status_dict->description,
        'request approval',
        'new status is request approval'
    );
    ok( !$test->set_status_wash_requied_if_needed(), 'wash not needed' );
    is(
        $test->current_instrument_status,
    ...
    ok(
        $test->set_status_wash_requied_if_needed(),
...

{
    lives_ok {
        $test =
          $schema->resultset('Instrument')->find( { id_instrument => 67 } );
    }
    ...
    cmp_bag(
        [ $active_rs->get_column('id_run')->all() ],
        [ 5329, 5330 ],
        'Match the run ids'
    );
    ...
    lives_ok {
        $test =
          $schema->resultset('Instrument')->find( { id_instrument => 68 } );
    }
    'Create a new test object - HiSeq, one active run';

    $active_rs = $test->current_runs();
    is( $active_rs->count(), 1, 'Find one active run' );
    cmp_bag( [ $active_rs->get_column('id_run')->all() ],
        [4329], 'Match the run id' );

    lives_ok {
        $test =
          $schema->resultset('Instrument')->find( { id_instrument => 15 } );
    }
    ...
    lives_ok {
        $test =
          $schema->resultset('Instrument')->find( { id_instrument => 3 } );
    }
...

{
    ...
    my $name = 'new name';
    lives_ok {
        $i = $schema->resultset('Instrument')->create(
            {
                id_instrument        => 100,
    ...
    is(
        $i->current_instrument_status,
        'wash required',
        'initial instrument status is set'
    );
