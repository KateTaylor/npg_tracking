...
my $mock = {
    q(SELECT i.name AS instrument, r.id_run AS id_run, DATE(rs.date) AS date
        ...
        ORDER BY rs.date DESC:1000) => [
        { intrument => 'IL1002', id_run => 1000, date => '2008-01-03' },
...
};
{
    ...
    is( $model->runs_loaded(), 'test',
        'nothing fetched as runs_loaded already present' );
    $model->{runs_loaded} = undef;
    isa_ok( $model->runs_loaded(), 'ARRAY' );
    is( scalar @{ $model->runs_loaded() },
        3, 'correct number of elements in array' );
