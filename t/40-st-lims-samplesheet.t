...

{
    ...
    is( $ss->id_run, 10000,
        'run-level id_run as set, differs from Experiment Name' );
    ...
    is( $plexes[95]->default_tag_sequence,
        'GTCTTGGC', 'tag sequence of the last plex' );
...

{
    ...
    is(
        join( q[ ], map { $_->tag_index } @plexes ),
        '3 4 11 12 22 23',
        'children array sorted by tag_index'
    );

    is( $plexes[0]->tag_index, 3, 'tag index for the first plex' );
    is( $plexes[0]->default_tag_sequence,
        'ATCACGTT', 'tag sequence for the first plex' );
    is( $plexes[0]->is_control, 0, 'plex is not control' );
    is( $plexes[0]->sample_name,
        'library_1',
        'plex sample name from the extended set rather than from Sample_Name' );
    is( $plexes[0]->study_id, 55, 'plex study_id' );
    is(
        $plexes[0]->sample_reference_genome,
        'Enterococcus hirae (ATCC_9790)',
        'sample ref genome'
    );
    is( $plexes[0]->study_reference_genome,
        undef, 'study ref genome undefined' );

    is( $plexes[1]->tag_index, 4, 'tag index for the first plex' );
    is( $plexes[1]->default_tag_sequence,
        'CGATGTTT', 'tag sequence for the first plex' );
    is( $plexes[1]->is_control, 1, 'plex is control' );
    is( $plexes[1]->sample_name,
        'library_2',
        'plex sample name from the extended set rather than from Sample_Name' );
    is( $plexes[1]->study_id, 56, 'plex study_id' );
    is( $plexes[1]->sample_reference_genome,
        undef, 'sample ref genome undefined' );
    is(
        $plexes[1]->study_reference_genome,
...

{
    ...
    is( join( q[ ], map { $_->position } @lanes ),
        '1 2 3 4 5', 'children array sorted by position' );
...

{
    ...
    use_ok('st::api::lims');
    my $path =
      't/data/samplesheet/6946_extended.csv';    #extended MiSeq samplesheet
    my @ss_lanes =
      st::api::lims::samplesheet->new( id_run => 6946, path => $path )
      ->children;
    local $ENV{NPG_WEBSERVICE_CACHE_DIR} = q[t/data/samplesheet];
    my @xml_lanes = st::api::lims::xml->new( batch_id => 13994 )->children;
    my @methods = grep { $_ ne 'lane_id' && $_ ne 'lane_priority' }
      @st::api::lims::DELEGATED_METHODS;
    push @methods, 'is_pool';

    ok( $ss_lanes[0]->is_pool, 'lane is a pool' );
    is_deeply(
        _lane_hash( $ss_lanes[0],  @methods ),
        _lane_hash( $xml_lanes[0], @methods ),
...

{
    ...
    is_deeply(
        _lane_hash( $ss_lanes[6],  @methods ),
        _lane_hash( $xml_lanes[6], @methods ),
        'xml and samplesheet drivers give the same result for plexes'
    );
}
