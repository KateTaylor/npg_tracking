...
lives_ok { $test = $schema->resultset(q{RunStatus})->new( {} ) }
...
my $run = $row->run();
is(
    $run->current_run_status_description(),
    q{run in progress},
    q{current run status correct}
);
...
like(
    ...
    qr{run[ ]in[ ]progress[ ]for[ ]run[ ]IL1_1},
    q{description saved in event looks correct}
);
