...

{
    ...
    is(
        $model->id_user(),
        $util->requestor->id_user(),
        'fixture user ids match'
    );
...

{
    ...
    is( $util->requestor->is_member_of('results'),
        undef, 'not a member of group' );
...

{
    ...
    like(
        $EVAL_ERROR,
        qr/not\ open\ for\ subscription/mix,
        'fail to subscribe to non-public group'
    );
}
