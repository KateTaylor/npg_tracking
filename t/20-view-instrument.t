...

{
    ...
    ok( $util->test_rendered( $str, 't/data/rendered/instrument/11.html' ),
        'read instrument' );
}

{
    ...
    ok( $util->test_rendered( $str, 't/data/rendered/instrument/13.xml' ),
        'read_xml renders ok' );
}
...

{
    ...
    unlike(
        $str,
        qr/not\ authorised/,
        'engineer authorised for group status update'
    );
...

{
    ...
    like( $str, qr{image/png.*PNG}smx,
        'instrument graphical utilisation (new-style)' );
}

{
    ...
    like( $str, qr{image/png.*PNG}smx,
        'instrument graphical utilisation by hour' );
}
...

{
    ...
    my $expected =
      GD::Image->new( File::Spec->catfile( $image_dir, 'key.png' ) );
    my @lines = split "\n", $str;
    shift @lines;
    shift @lines;
    shift @lines;
...

{
    ...
    my $expected =
      GD::Image->new( File::Spec->catfile( $image_dir, 'HS3.png' ) );
    my @lines = split "\n", $str;
    shift @lines;
    shift @lines;
    shift @lines;
...

{
    ...
    ok(
        $util->test_rendered(
            $str, 't/data/rendered/instrument_status;add-cis2.html'
        ),
        'render of add ok for current instrument status of down'
    );
}
