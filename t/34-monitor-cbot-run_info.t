...
my $user_agent = t::useragent->new(
    {
        is_success => 1,
        mock       => {
q{http://cBot1.internal.sanger.ac.uk/RunInfo/091111_OEM-0B6VO7E14KH_0001}
              => q{t/data/cbot/cBot1/RunInfo/091111_OEM-0B6VO7E14KH_0001.xml},
        },
    }
);
...
is( $test->protocol_name(),
    'PE_Amp_Lin_Block_Hyb_v7.0', 'Return protocol name' );

my $current_run_info = $test->current_run_info();
my $xml_content      = slurp "t/data/cbot/$dummy_cbot/RunInfo/$run_label.xml";

isa_ok( $current_run_info, 'XML::LibXML::Document', 'current_run_info output' );

is( $test->latest_run_info->toString(),
    $xml_content, 'Store the latest run info' );
