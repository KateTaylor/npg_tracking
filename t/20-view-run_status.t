...
my $mock = {
    q(SELECT id_user, username FROM user WHERE id_user=?:1) =>
      [ { id_user => 1, username => 'public' } ],
...
q(SELECT ug.id_usergroup, ug.groupname, ug.is_public, ug.description, uug.id_user_usergroup FROM usergroup ug, user2usergroup uug WHERE uug.id_user = ? AND ug.id_usergroup = uug.id_usergroup:1)
      => [ { id_usergroup => 102, groupname => 'public' } ],
    q(SELECT id_usergroup FROM usergroup WHERE groupname = ?:,public) =>
      [ [102] ],
q(SELECT ug.id_usergroup, ug.groupname, ug.is_public, ug.description, uug.id_user_usergroup FROM usergroup ug, user2usergroup uug WHERE uug.id_user = ? AND ug.id_usergroup = uug.id_usergroup:2)
      => [ { id_usergroup => 103, groupname => 'pipeline' } ],
q(SELECT ug.id_usergroup, ug.groupname, ug.is_public, ug.description, uug.id_user_usergroup FROM usergroup ug, user2usergroup uug WHERE uug.id_user = ? AND ug.id_usergroup = uug.id_usergroup:3)
      => [ { id_usergroup => 104, groupname => 'engineers' } ],
q(SELECT ug.id_usergroup, ug.groupname, ug.is_public, ug.description, uug.id_user_usergroup FROM usergroup ug, user2usergroup uug WHERE uug.id_user = ? AND ug.id_usergroup = uug.id_usergroup:4)
      => [ { id_usergroup => 105, groupname => 'loaders' } ],
};
...

{
    ...
    eval { $rs->render(); };
    ok( $EVAL_ERROR,
'create_xml and no pipeline, and create and no engineers or loaders croaked'
    );
}
{
    my $mock = {
...
q(INSERT INTO run_status (id_run,date,id_run_status_dict,id_user,iscurrent) VALUES (?,now(),?,?,1):,NULL,NULL,3)
          => 1,
...
q(SELECT u.id_usergroup, u.groupname, u.is_public, u.description FROM usergroup u, event_type_subscriber ets WHERE ets.id_event_type = ? AND ets.id_usergroup = u.id_usergroup:1)
          => [ {} ],
q(SELECT id_user FROM user2usergroup uug WHERE uug.id_usergroup = ?:NULL)
          => [ { id_user => 3 } ],
        q(SELECT id_user, username FROM user WHERE id_user=?:3) =>
          [ { id_user => 3, username => 'test' } ],
...
}
{
    my $mock = {
q(SELECT ug.id_usergroup, ug.groupname, ug.is_public, ug.description, uug.id_user_usergroup FROM usergroup ug, user2usergroup uug WHERE uug.id_user = ? AND ug.id_usergroup = uug.id_usergroup:3)
          => [ { id_usergroup => 104, groupname => 'loaders' } ],
        q(SELECT id_usergroup FROM usergroup WHERE groupname = ?:,public) =>
          [ [101] ],
    };
    my $util = t::util->new(
...

{
    ...
    ok(
        $util->test_rendered(
            $render, 't/data/rendered/run_status/create_xml.xml'
        ),
...

{
    ...
    ok(
        $util->test_rendered(
            $str, 't/data/rendered/run_status/t_request_create.xml'
        ),
        q{returned xml is correct}
    );
