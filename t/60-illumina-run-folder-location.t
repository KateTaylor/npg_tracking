...
{    # no short_reference method
    ...
    throws_ok { $test->runfolder_path } qr/Not enough information/,
      ' arrrgh no short_reference method'
}
...
{    # short_reference method but undef
    my $testc =
      Moose::Meta::Class->create_anon_class(
        roles => [qw(npg_tracking::illumina::run::folder::location)], );
    $testc->add_attribute(
        q[short_reference] => ( q[is] => q[ro], q[default] => undef ) );
    my $test;
    lives_ok {
...
{    # short_reference method, with suitable folder
    ...
    lives_and {
        is $test->runfolder_path, qq($incoming/101217_HS11_05636_A_90061ACXX);
    }
    ' run folder found';
}
...
{    # short_reference method, with suitable folder
    ...
    lives_ok {
        $test = $testc->new_object(
            q[short_reference] => q[100914_HS3_05281_A_205MBABXX] );
    }
    'anon class with short_reference for folder';
    lives_and {
        is $test->runfolder_path, qq($incoming/100914_HS3_05281_A_205MBABXX);
    }
    ' run folder found';
}
...
{    # schema method but undef, runfolder can be found
    my $testc =
      Moose::Meta::Class->create_anon_class(
        roles => [qw(npg_tracking::illumina::run::folder::location)], );
    $testc->add_attribute(
        q[npg_tracking_schema] => ( q[is] => q[ro], q[default] => undef ) );
    $testc->add_attribute(
        q[short_reference] => ( q[is] => q[ro], q[default] => undef ) );
    my $test;
    lives_ok {
...
my $testrun =
  Test::MockObject::Extends->new(q(npg_tracking::Schema::Result::Run));
$testschema->mock(
    q(find),
    sub {
        my ( $self, $id_run ) = @_;
...
{    # schema, no id_run, but with short ref and a folder
    ...
    lives_ok {
        $test = $testc->new_object(
            q[short_reference]     => q[100914_HS3_05281_A_205MBABXX],
            q[npg_tracking_schema] => $testschema
        );
    }
...
'anon class, schema, id_run, with short_reference for folder - but no staging tag';
    throws_ok { $test->runfolder_path }
    qr/NPG tracking reports run \d* no longer on staging/,
      ' no run folder found';
}
...
{    # schema, no short ref, id_run, staging tag, glob, folder_name
    $testrun->mock( q(folder_path_glob),
        sub { return q[t/data/long_info/{export,nfs}/sf20/ILorHSany_sf20/*]; }
    );
    $testrun->mock( q(folder_name),
...
'anon class, schema, id_run, no short_reference, staging tag and folder name and glob';
    ...
    ' run folder found and does not contain a double slash';
}
