...
use_ok('npg::model');
{
    ...
    is( $model->model_type(), 'model',
        'entity type returns last part of reference to object' );
}

{
    ...
    isa_ok( $run_tags->[0], 'npg::model::tag',
        'array objects are npg::model::tag objects' );
}

{
    ...
    is(
        $util->dbh->do(
"update instrument set instrument_comp='$hostname' where id_instrument=3"
        ),
        1,
        'one row in test db updated'
    );

    is( $model->location_is_instrument(),
        undef, q{undef returned with no headers set} );
    $ENV{HTTP_X_FORWARDED_FOR} = qq{$addr, 127.0.0.2};
    $ENV{REMOTE_ADDR}          = q{127.0.0.5};
    is( $model->location_is_instrument(),
        3, q{instrument found from HTTP_X_FORWARDED_FOR} );
    ...
    is( $model->location_is_instrument(),
        3, q{instrument found from REMOTE_ADDR} );
}
