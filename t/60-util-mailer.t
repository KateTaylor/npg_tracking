...
}
{
    ...
    qr{Missing[ ]initializer[ ]label[ ]for[ ]npg::util::mailer:[ ]'to'\.},
      q{croak as missing 'to' in args};
}
{
    my $mailer;
    throws_ok {
    ...
    qr{Missing[ ]initializer[ ]label[ ]for[ ]npg::util::mailer:[ ]'from'\.},
      q{croak as missing 'from' in args};
}
{
    my $mailer;
    throws_ok {
    ...
    qr{Missing[ ]initializer[ ]label[ ]for[ ]npg::util::mailer:[ ]'subject'\.},
      q{croak as missing 'subject' in args};
}
{
    my $mailer;
    throws_ok {
    ...
    qr{Missing[ ]initializer[ ]label[ ]for[ ]npg::util::mailer:[ ]'body'\.},
      q{croak as missing 'body' in args};
}
{
    my $mailer = npg::util::mailer->new(
        {
            ...
            subject => q{A test subject},
            body =>
              qq{<xml>This is some test stuff\n\nLots of test stuff</xml>},
    ...
    isa_ok( $mailer, q{npg::util::mailer},
        q{$mailer (including optional headers)} );
    lives_ok { $mailer->mail(); } q{no croak mailing};
    my $mail = $util->parse_email( $util->{emails}->[1], 1 );
    is(
        $mail->{to},
        qq{receive\@tester.org, second_recepient\@tester.org\n},
        q{to is correct}
    );
    is( $mail->{from},    qq{send\@tester.org\n}, q{from is correct} );
    is( $mail->{subject}, qq{A test subject\n},   q{subject is correct} );
    ...
    is(
        $mail->{cc},
        qq{one_cc\@tester.org, two_cc\@tester.org\n},
        q{cc is correct}
    );
    is(
        $mail->{bcc},
        qq{one_bcc\@tester.org, two_bcc\@tester.org\n},
        q{bcc is correct}
    );
