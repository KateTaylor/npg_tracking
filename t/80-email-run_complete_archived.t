...

{
    my $object;
    lives_ok {
        $object = npg::email::run::complete_archived->new(
            {
    ...
    is( $study_lane_followers, $hash_projects_followers,
        q{study_lane_followers is returning the test hash} );
    ...
    isa_ok(
        $run_result,
        q{npg_tracking::Schema::Result::Run},
        q{correct run object returned from get_run(95)}
    );
    is( $run_result->batch_id(), 62, q{correct batch_id for run} );

    isa_ok( $object->email_body_store(),
        q{ARRAY}, q{email_body_store is an arrayref} );

    lives_ok { $object->run(); } q{no croak running $object->run()};

    my $email1_body = qq{has a status of run complete in NPG tracking.\n\n};
    $email1_body .=
qq{From the database system, the following project has the following lanes on this run\n\nhuman\n\n};
    $email1_body .= qq{Lanes:\n\n};
    $email1_body .= qq{Lane - 1 : Library - 1k\n};
    $email1_body .= qq{Lane - 2 : Library - 1k\n};
    $email1_body .= qq{Lane - 3 : Library - 1k\n\n};
    $email1_body .=
      qq{You will be notified when this run reaches run archived.\n\n};
    $email1_body .=
qq{In the mean time, you can check the status of the run through NPG:\n\n};
    $email1_body .= qq{http://npg.sanger.ac.uk/perl/npg/run/};

    my $email_hash = $util->parse_email( $util->{emails}->[0] );
    like(
        $email_hash->{subject},
        qr{Run[ ]\d+[ ]-[ ]run[ ]complete[ ]-[ ]human},
        q{email1 subject correct}
    );
    is(
        $email_hash->{to},
        q{human@sanger.ac.uk, monkey@sanger.ac.uk} . qq{\n},
        q{email1 to correct}
    );
    is(
        $email_hash->{from},
        q{srpipe@sanger.ac.uk} . qq{\n},
