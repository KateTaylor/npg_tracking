...

{
    ...
    is( $test->current_run_status_description(),
        'run complete', '  Run status updated' );

    is( $test->run_db_row->actual_cycle_count(), 43, '  Cycle count updated' );
}

{
    ...
    lives_ok { $test->read_long_info(0) }
    'Call read_long_info method without error';
    move( "$mock_path/_Data", "$mock_path/Data" ) or croak "Error $OS_ERROR";

    is( $test->run_db_row->is_tag_set('single_read'),
        1, '  \'single_read\' tag is set on this run' );
    is( $test->run_db_row->is_tag_set('multiplex'),
        0, '  \'multiplex\' tag is not set on this run' );

    is( $test->run_db_row->is_tag_set('rta'),
        0, '  \'rta\' tag is not set on this run' );
    ...
    is( $test->run_db_row->is_tag_set('paired_read'),
        1, '  \'paired_read\' tag is set on that run' );
    is( $test->run_db_row->is_tag_set('multiplex'),
        1, '  \'multiplex\' tag is set on that run' );
    is( $test->run_db_row->is_tag_set('rta'),
        1, '  \'rta\' tag is set on that run' );
