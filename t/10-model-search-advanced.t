...
my $mock = {
    ...
    q(SELECT DISTINCT run.id_run FROM run WHERE run.id_run = 10:) =>
      [ { id_run => 10 } ],
...
};
{
    ...
    is( scalar @{$results},
        1, '1 result - creates a query via a joining table' );
...
}
{
    ...
    my $results = $model->advanced_search();
    is( $results, 'test',
        'no query made if $model->{results} already present' );
...

{
    ...
    my $results = $model->advanced_search();
    is( scalar @{$results},
        2, '2 results - creates a query where only 2 tables were needed' );
    my @keys = @{ $model->{'result_keys'} };
    is( scalar @keys, 3,        '3 keys' );
...

{
    ...
    my $results = $model->advanced_search();
    is( scalar @{$results},
        1, '1 result - creates a query where multiple froms are used' );
    my @keys = @{ $model->{'result_keys'} };
    is( scalar @keys, 3,        '3 keys' );
    is( $keys[0],     'id_run', 'id_run in correct position in array' );
...

{
    ...
    my $results = $model->advanced_search();
    is( scalar @{$results},
        1, '1 result expected, as id_run enforced to be first' );
...
}
{
    ...
    my $results = $model->advanced_search();
    is( scalar @{$results},
        2, '2 results - creates a query from only a single table' );
    my @keys = @{ $model->{'result_keys'} };
    is( scalar @keys, 2,          '2 keys' );
    is( $keys[1],     'batch_id', 'batch_id in correct position in array' );
    is( $results->[1]->[1],
        244, 'batch_id ok and in correct position in array' );

}

# changed as util now singleton within scope of whole test file - change to fixtures asap
$mock->{q(SELECT DISTINCT run.id_run FROM run:)} =
  [ { id_run => 1 }, { id_run => 2 } ];
$mock->{
q(SELECT tr.id_run, t.tag FROM tag t, tag_run tr WHERE t.id_tag = tr.id_tag AND tr.id_run in (1,2) ORDER BY tr.id_run:)
} = [ { id_run => 1, tag => 'good' }, { id_run => 1, tag => 'superb' } ];
{
    my $cgi = CGI->new();
