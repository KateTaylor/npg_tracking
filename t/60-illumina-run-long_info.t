...
$ENV{TEST_DIR} = $basedir
  ;    #so when npg_tracking::illumina::run::folder globs the test directory
...
{
    my $long_info;
    lives_ok { $long_info = test::long_info->new( { id_run => 1234 } ); }
    q{created role_test object ok};

    create_staging();

    is( $long_info->use_bases(), q{Y76,I8,y76},
        q{use_bases string is correct} );
    lives_ok { $long_info = test::long_info->new( { id_run => 1234 } ); }
    q{created role_test object ok};
    is( $long_info->read_config_string(),
        q{Y76,I8,y76}, q{read_config_string returns same as use_bases} );
    ...
    lives_ok { $long_info = test::long_info->new( { id_run => 1234 } ); }
    q{created role_test object ok};
    is( $long_info->expected_cycle_count(),
        160, q{correct number of expected cycles} );
    lives_ok { $long_info = test::long_info->new( { id_run => 1234 } ); }
    q{created role_test object ok};
    is( $long_info->cycle_count(),
        160, q{cycle count returns the same as expected_cycle_count} );

    lives_ok { $long_info = test::long_info->new( { id_run => 1234 } ); }
    q{created role_test object ok};
    is( $long_info->tilelayout_columns(),
        2, q{correct number of tilelayout_columns} );
    lives_ok { $long_info = test::long_info->new( { id_run => 1234 } ); }
    q{created role_test object ok};
    is( $long_info->tilelayout_rows(),
        60, q{correct number of tilelayout_rows} );
    lives_ok { $long_info = test::long_info->new( { id_run => 1234 } ); }
    q{created role_test object ok};
...
{
    my $long_info;
    lives_ok { $long_info = test::long_info->new( { id_run => 5281 } ); }
    ...
    lives_ok { $long_info = test::long_info->new( { id_run => 5281 } ); }
    q{created role_test (HiSeq run 5281, RunInfo.xml) object ok};
    cmp_ok( $long_info->use_bases, 'eq', 'Y100,y100',
        'correct use_bases string' );
    ...
    is_deeply( $long_info->lane_tile_clustercount(),
        $test_lane_tile_clustercount, q{lane_tile_clustercount as expected} );

    $long_info = undef;
    lives_ok { $long_info = test::long_info->new( { id_run => 5636 } ); }
    q{created role_test (HiSeq run 5636, RunInfo.xml) object ok};
    ...
    lives_ok { $long_info = test::long_info->new( { id_run => 5636 } ); }
    q{created role_test (HiSeq run 5636, RunInfo.xml) object ok};
    cmp_ok( $long_info->use_bases, 'eq', 'Y101,y101',
        'correct use_bases string' );
    note( $long_info->runfolder_path );
...

{
    my $long_info;
    my $rfpath =
q(t/data/long_info/nfs/sf20/ILorHSany_sf20/incoming/120110_M00119_0068_AMS0002022-00300);
    lives_ok {
        $long_info = test::long_info->new( { runfolder_path => $rfpath } );
    }
    q{create test role for dual index paired};
    cmp_ok( $long_info->expected_cycle_count,
        '==', 318, 'expected_cycle_count' );
    lives_ok {
        $long_info = test::long_info->new( { runfolder_path => $rfpath } );
    }
    q{create test role for dual index paired};
    lives_and {
        cmp_deeply(
            [ $long_info->read_cycle_counts ],
            [ 151, 8, 8, 151 ],
            'read_cycle_counts match'
        );
    }
    'read_cycle_counts live and match';
    lives_ok {
        $long_info = test::long_info->new( { runfolder_path => $rfpath } );
    }
    q{create test role for dual index paired};
    lives_and {
        cmp_deeply(
            [ $long_info->read1_cycle_range ],
            [ 1, 151 ],
            'read1_cycle_range matches'
        );
    }
    'read1_cycle_range lives and matches';
    lives_ok {
        $long_info = test::long_info->new( { runfolder_path => $rfpath } );
    }
    q{create test role for dual index paired};
    lives_and {
        cmp_deeply(
            [ $long_info->indexing_cycle_range ],
            [ 152, 167 ],
            'indexing_cycle_range matches'
        );
    }
    'indexing_cycle_range lives and matches';
    lives_ok {
        $long_info = test::long_info->new( { runfolder_path => $rfpath } );
    }
    q{create test role for dual index paired};
    lives_and {
        cmp_ok( $long_info->index_length, '==', 16, 'index_length matches' );
    }
    'index_length lives and matches';
    lives_ok {
        $long_info = test::long_info->new( { runfolder_path => $rfpath } );
    }
    q{create test role for dual index paired};
    lives_and {
        cmp_deeply(
            [ $long_info->read2_cycle_range ],
            [ 168, 318 ],
            'read2_cycle_range matches'
        );
    }
    'read2_cycle_range lives and matches';
    lives_ok {
        $long_info = test::long_info->new( { runfolder_path => $rfpath } );
    }
    q{create test role for dual index paired};
    lives_and { ok( $long_info->is_indexed, 'is_indexed ok' ); }
    'is_indexed lives and ok';
    lives_ok {
        $long_info = test::long_info->new( { runfolder_path => $rfpath } );
    }
    q{create test role for dual index paired};
    lives_and { ok( $long_info->is_paired_read, 'is_paired_read ok' ); }
    'is_paired_read lives and ok';
}
1;
