...
foreach my $pa (
    [ 'test', 'using mocked data', q[t/data/test45] ],
  ...
  )
{
  ...
  SKIP: {
        ...
        }
        {
            ...
            is( scalar $lims1->associated_lims,
                0, 'no associated lims for a lane' );

            cmp_ok( $lims1->library_id, q(==), 57440,
                'lib id from first st lane' );
            cmp_ok( $lims1->library_name, q(eq), 'PD3918a 1',
                'lib name from first st lane' );

            my $insert_size;
            lives_ok { $insert_size = $lims1->required_insert_size }
            'insert size for the first lane';
            is( keys %{$insert_size}, 1, 'one entry in the insert size hash' );
            is( $insert_size->{ $lims1->library_id }->{q[from]},
                300, 'required FROM insert size' );
            is( $insert_size->{ $lims1->library_id }->{q[to]},
                400, 'required TO insert size' );

            ok(
                !$lims1->sample_consent_withdrawn(),
                'sample consent not withdrawn'
            );
            ...
            is( $lims7->bait_name, undef,
                'bait name undefined for spiked phix plex' );

            my $lims8 = st::api::lims->new(
                batch_id  => 15728,
                position  => 2,
                tag_index => 3
            );
            ok(
                $lims8->sample_consent_withdrawn(),
                'sample 1299723 consent withdrawn'
            );
