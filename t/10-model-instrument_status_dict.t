...

{
    ...
    is( $model->id_instrument_status_dict(),
        1, 'initialised by description ok' );
}
...

{
    ...
    isa_ok( $model, $ISD, '$model' );
    is( $model->description(), 'down',
        'initialised by id_instrument_status_dict ok' );
...

{
    ...
    isa_ok( $isds, 'ARRAY',
        'unprimed cache $model->instrument_status_dicts()' );
    is( ( scalar @{$isds} ),
        11, 'unprimed cache number of instrument_status_dicts' );
    is_deeply(
        $isds,
        $model->instrument_status_dicts(),
        'primed cache insturment_status_dicts'
    );
}

{
    ...
    is_deeply( $model->instruments(), $instruments,
        'primed cache instruments' );
...

{
    ...
    is(
        join( q[;],
            sort keys %npg::model::instrument_status_dict::SHORT_DESCRIPTIONS ),
'down;down for repair;down for service;planned maintenance;planned repair;planned service;request approval;up;wash in progress;wash performed;wash required',
    ...
    is(
        $npg::model::instrument_status_dict::SHORT_DESCRIPTIONS{
            'wash required'},
        'wash', 'short status description for "wash required"'
    );
}
