...

{
    ...
    my $lid = $lims->library_id;
    my $insert_size;
    lives_ok { $insert_size = $lims->required_insert_size }
    'insert size for the first lane lives';
    ...
    ok( !exists $insert_size->{2798525}->{q[from]},
        'no required FROM insert size' );
    is( $insert_size->{2798525}->{q[to]}, 500, 'required TO insert size' );

    $lims =
      st::api::lims->new( batch_id => 4775, position => 7, tag_index => 2 );
    lives_ok { $insert_size = $lims->required_insert_size }
    'insert size for the plex lives';
    is( keys %{$insert_size}, 1, 'one entry in the insert size hash' );
    is( $insert_size->{2798524}->{q[from]}, 40, 'required FROM insert size' );
    is( $insert_size->{2798524}->{q[to]},   50, 'required TO insert size' );
}
...
{
    my $lims =
      st::api::lims->new( batch_id => 14706, position => 1, tag_index => 2 );
    my $insert_size;
    lives_ok { $insert_size = $lims->required_insert_size }
    'insert size for the plex without lib id lives';
    ...
    lives_ok { $insert_size = $lims->required_insert_size }
    'insert size for the pool with plexes without lib ids lives';
    is( join( q[ ], sort keys %{$insert_size} ),
        '1 2 3 4 5', 'tag index entries in the insert size hash' );
