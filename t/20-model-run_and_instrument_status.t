...

{
    ...
    my $instr_status = npg::model::instrument->new(
        { util => $util, id_instrument => $model->{run}->id_instrument } )
      ->current_instrument_status->instrument_status_dict->description;

    is(
        $instr_status,
        'planned maintenance',
        'planned maintenance is current instrument status in database'
    );
    $util->catch_email($model);

    lives_ok { $model->create() }
    'run status run complete created for id_run 16';

    $instr_status = npg::model::instrument->new(
        { util => $util, id_instrument => $model->{run}->id_instrument } )
      ->current_instrument_status->instrument_status_dict->description;
    is(
        $instr_status,
        'planned maintenance',
        'no automatic change from planned maintenance on run complete)'
    );
    ...
    $util->catch_email($model);
    lives_ok { $model->create() }
    'run status run mirrored created for id_run 16';
...
qr/Instrument IL11 \"planned repair\" status cannot follow current \"down for repair\" status/,
    ...
    'request approval can follow down';
    ...
    is(
        $instr_status,
        'request approval',
        'request approval is current instrument status'
    );
    ...
    'planned repair can follow request approval';

    is(
        npg::model::instrument->new(
            { util => $util, id_instrument => $id_instrument, }
          )->current_instrument_status->instrument_status_dict->description,
        'planned repair',
    ...
    $run = npg::model::run->new( { id_run => 16, util => $util, } );
    is( $run->id_instrument, $id_instrument,
        'new run is on the instrument under test' );
    $model = npg::model::run_status->new(
        {
            util               => $util,
            id_run             => 16,
...

{
    ...
    'instrument status for request approval created';
    ...
    'instrument status for planned repair created';
...
diag 'Status change for runs on HiSeq instruments';
{
    ...
    my $i = npg::model::instrument->new(
        { util => $util, id_instrument => $id_instrument, } );
    is(
        $i->current_instrument_status->instrument_status_dict->description,
        'planned repair',
        'planned repair is current instrument status'
    );
    ...
    is(
        $i->current_instrument_status->instrument_status_dict->description,
        'down for repair',
        'instrument switched to down as the other slot is free'
    );
}

{
    ...
    'instrument status for wash in progress created';
    ...
    'instrument status for wash performed created';

    my $in = npg::model::instrument->new(
        { util => $util, id_instrument => $id_instrument, } );
    ...
    $util->catch_email($model);
    lives_ok { $model->create() }
    'run status run cancelled created for a run in one of the slots';
    ...
    'instrument status for planned service created';

    is(
        $in->current_instrument_status->instrument_status_dict->description,
        'planned service',
'planned service is current instrument status since one of the slots is not idle '
    ...
    'instrument status for down for repair created';
    ...
    $util->catch_email($model);
    lives_ok { $model->create() }
    'run status run cancelled created for a run in the other slot';
    ...
    is(
        $in->current_instrument_status->instrument_status_dict->description,
        'down for service',
'down for service is current instrument status since both slots are idle'
    );
}

{
    ...
    'instrument status for request approva created';
    my $instr_status = npg::model::instrument->new(
        { util => $util, id_instrument => $id_instrument, } )
      ->current_instrument_status->instrument_status_dict->description;
    is(
        $instr_status,
        'request approval',
        'request approva is current instrument status'
    );
    ...
    $util->catch_email($model);
    lives_ok { $model->create() }
    'run status run pending created for a run in one of the slots';
...
'planned repair is current instrument status since one of the slots is not idle '
    ...
    $util->catch_email($model);
    lives_ok { $model->create() }
    'run status run mirrored created for a run in one of the slots';
    is(
        $in->current_instrument_status->instrument_status_dict->description,
        'planned repair',
