...
my $ua       = t::useragent->new(
    {
        ...
        mock       => {
            $lims_url
              . q{/batches/1027} => q{t/data/npg_api/st/batches/1027.xml},
            $lims_url
              . q{/batches/1826} => q{t/data/npg_api/st/batches/1826.xml},
            $lims_url
              . q{/samples/10014} => q{t/data/npg_api/st/samples/10014.xml},
        },
    }
);
...
{
    is( npg::api::request->cache_dir_var_name,
        q[NPG_WEBSERVICE_CACHE_DIR], 'class attr for var name' );
    is(
        npg::api::request->new()->cache_dir_var_name,
        q[NPG_WEBSERVICE_CACHE_DIR],
        'class attr for var name works for an instance as well'
    );
    is(
        npg::api::request->new()->cache_dir_var_name,
        q[NPG_WEBSERVICE_CACHE_DIR],
...

{
    ...
    throws_ok { $r->make(q[]) } qr/Uri is not defined/,
      'error if an empty uri given';

    local $ENV{$VAR_NAME} = q[/dome/non-existing];
    throws_ok { $r->make( q[dodo], q[GET] ) }
    qr/Cache directory \/dome\/non-existing does not exist/,
      'error when a cache directory does not exist';

    local $ENV{$VAR_NAME} = q[Changes];
    throws_ok { $r->make( q[dodo], q[GET] ) } qr/is not a directory/,
      'error when a cache directory is not a directory';
}

{
    ...
    is(
        $r->_create_path(
q{http://news.bbc.co.uk/sport1/hi/football/world_cup_2010/matches/match_01}
        ),
...

{
    ...
    is( $req->header(q[X-username]),
        getlogin, 'username set in the request header' );
}

{
    my $util = npg::api::util->new( { useragent => $ua, max_retries => 1, } );
    my $sample_id = 10014;
    ok( -e catfile( $TEST_CACHE, q[st], q[samples], $sample_id . q[.xml] ),
        'test prerequisite OK' );
    is(
        st::api::sample->new(
            {
                id   => $sample_id,
        ...
        q[104A_Sc_YPS128],
        'sample fetched,  name correct'
    );

    my $dir = tempdir( CLEANUP => 1 );
    local $ENV{$VAR_NAME} = $dir;
    throws_ok { st::api::sample->new( { id => $sample_id, } )->name }
    qr/is not in the cache/,
      'error when fetching from a cache a resource that is not there';
}

{
    ...
    lives_ok {
        st::api::batch->new( { id => $batch_id, util => $util, } )->read();
    }
    'batch retrieved';
    ...
    lives_ok {
        st::api::batch->new( { util => $util, id => $batch_id } )->read();
    }
    ...
    'call to save a second batch lives';
...

{
    my $batch_id = 3022;
    my $original =
      catfile( $TEST_CACHE, q[st], q[batches], $batch_id . q[.xml] );
    ...
    cmp_deeply( $batch1, $batch2,
        'the same object returned from cache and test repository' );
}

{
    ...
    lives_ok {
        $returned =
          npg::api::request->new()->make( $base_url . q[/run/4913], q[POST] );
    }
    'post request lives';
