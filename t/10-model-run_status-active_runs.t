...
my $mock = {
    q(SELECT id_run, date
               ...
               ORDER BY date:) => [
        { id_run => 1000, date => 'first day' },
        { id_run => 1000, date => 'second day' }
    ],
};
...

{
    ...
    is( $active_runs_over_last_30_days,
        1, 'does not fetch active runs from database if present already' );
    $model->{'active_runs_over_last_30_days'} = undef;
    $active_runs_over_last_30_days = $model->active_runs_over_last_30_days();
    isnt( $active_runs_over_last_30_days,
        undef, 'fetches active runs from database if not already present' );
    is( $active_runs_over_last_30_days->[0]{date},
        'first day', 'Testing retrieval of information is as expected' );
    is( $active_runs_over_last_30_days->[1]{date},
        'second day', 'Testing retrieval of information is as expected' );
}
1;
