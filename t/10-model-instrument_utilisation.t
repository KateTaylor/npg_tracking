...

{
    ...
    ok( $total_number_of_all_object,
        "Number of instrument_utilisations = $total_number_of_all_object" );
}
{
    my $model = npg::model::instrument_utilisation->new(
        { util => $util, date => '2008-01-01' } );
    isa_ok( $model, 'npg::model::instrument_utilisation', '$model' );
    is( $model->id_instrument_utilisation(),
        1, '$model->id_instrument_utilisation()' );
    is( $model->total_insts(), 10, '$model->total_insts()' );
    is( $model->perc_utilisation_total_insts(),
        '100.00', '$model->perc_utilisation_total_insts()' );
    is( $model->perc_uptime_total_insts(),
        '100.00', '$model->perc_uptime_total_insts()' );
    is( $model->official_insts(), 8, '$model->official_insts()' );
    is( $model->perc_utilisation_official_insts(),
        '100.00', '$model->perc_utilisation_official_insts()' );
    is( $model->perc_uptime_official_insts(),
        '100.00', '$model->perc_uptime_official_insts()' );
    is( $model->prod_insts(), 7, '$model->prod_insts()' );
    is( $model->perc_utilisation_prod_insts(),
        '100.00', '$model->perc_utilisation_prod_insts()' );
    is( $model->perc_uptime_prod_insts(),
        '100.00', '$model->perc_uptime_prod_insts()' );
}
{
    my $model = npg::model::instrument_utilisation->new(
        { util => $util, id_instrument_utilisation => 1 } );
    isa_ok( $model, 'npg::model::instrument_utilisation', '$model' );
...
}
{
    ...
    my $total_insts_30_days = [
        [qw(2008-01-01 10 100.00 100.00)], [qw(2008-01-02 10 100.00 100.00)],
        [qw(2008-01-03 10 100.00 100.00)], [qw(2008-01-04 10 100.00 100.00)],
        [qw(2008-01-05 10 100.00 100.00)],
    ];
    is_deeply( $data, $total_insts_30_days,
        '$model->graph_data_total_insts()' );
    eval { $data = $model->table_data_official_insts(); };
    is( $EVAL_ERROR, q{},
        'no croak on obtaining $model->graph_data_official_insts()' );
    my $official_insts_30_days = [
        [qw(2008-01-01 8 100.00 100.00)], [qw(2008-01-02 9 100.00 100.00)],
        [qw(2008-01-03 9 90.00 100.00)],  [qw(2008-01-04 9 87.53 112.00)],
        [qw(2008-01-05 8 100.00 100.00)],
    ];
    is_deeply( $data, $official_insts_30_days,
        '$model->graph_data_official_insts()' );
    eval { $data = $model->table_data_prod_insts(); };
    is( $EVAL_ERROR, q{},
        'no croak on obtaining $model->graph_data_prod_insts()' );
    $total_insts_30_days = [
        [qw(2008-01-01 7 100.00 100.00)], [qw(2008-01-02 8 100.00 100.00)],
        [qw(2008-01-03 8 100.00 100.00)], [qw(2008-01-04 8 100.00 100.00)],
    ...
    like(
        $EVAL_ERROR,
        qr/no\ instrument\ grouping\ provided/,
        'croaked last_30_days method call as no string provided'
    );
    eval { $data = $model->last_30_days( { insts => 'iwantsomeinsts', } ); };
    like(
        $EVAL_ERROR,
        qr/Unknown\ column\ 'iwantsomeinsts'\ in\ 'field\ list'/,
'croaked last_30_days method call as string does not match a column header'
    );
