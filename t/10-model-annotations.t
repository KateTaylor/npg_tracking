...

{
    ...
    is(
        ...
        'Lack of clusters - run cancelled',
        'get comment'
    );
...

{
    ...
    my $dt_then = DateTime->new(
        year   => int substr( $create_time, 0,  4 ),
...

{
    ...
    is(
        $model2->id_annotation(),
        $model->id_annotation(),
        'id_annotation unchanged on update'
    );
    is( $model2->id_user(), $model->id_user(), 'id_user unchanged on update' );
    is(
        $model2->comment(),
        'Revised new annotation comment',
        'comment changed on update'
    );
}
