...

{
    ...
    is(
        $model->instrument_status->comment,
        'initial setup',
        'correct coment for the status'
    );

    isa_ok( $model->annotation, 'npg::model::annotation' );
    is( $model->annotation->id_annotation(),
        undef, 'no id_annotation create yet' );
    ...
    is( $model->id_instrument_status_annotation,
        3, 'instrument_status_annotation created with id 3' );
}
1;
