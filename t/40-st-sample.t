...
'AB GO (grandmother) of the MGH meiotic cross. The same DNA was split into three aliquots (of which this is reaction A), processed in parallel, they were:  NspI cut, ligated with modified recuttable adaptor (ATTATGAGCACGACAGACGCCTGATCTRCATG and YAGATCAGGCGTCTGTCGTGCTCATAA), and PCR amplified.';
is( $sample->description(),
    $sample_description, 'Description field fetched ok' );
...
is(
    $sample->common_name(),
    'Salmonella enterica subsp. enterica serovar Java',
    'Common Name fetched ok'
);
is( $sample->public_name(), 'H083280501', 'public name fetched ok' );
is( $sample->strain(),      'S java',     'strain fetched ok' );

is(
    st::api::sample->new( { id => 1121926, } )->reference_genome(),
    'Schistosoma_mansoni (20100601)',
    'ref genome'
);

ok( !st::api::sample->new( { id => 11036, } )->consent_withdrawn(),
    q{consent not withdrawn for sample 11036} );
ok( !st::api::sample->new( { id => 1121926, } )->consent_withdrawn(),
    q{consent withdrawn not given for sample 1121926} );
ok( st::api::sample->new( { id => 1299723, } )->consent_withdrawn(),
    q{consent withdrawn for sample 1299723} );
