...
lives_ok {
    $test[0] = npg::email::event::status_change::run->new(
        {
...
lives_ok {
    $test[1] = npg::email::event::status_change::run->new(
        {
            id_event          => 24,
            schema_connection => $schema,
        }
...
throws_ok {
    npg::email::event::status_change::run->new(
        {
            event_row         => $schema->resultset('Event')->find(23),
            id_event          => 24,
            schema_connection => $schema,
...
throws_ok {
    my $fail = npg::email::event::status_change::run->new(
        {
            id_event          => 25,
            schema_connection => $schema,
        }
...
cmp_bag(
    $test[0]->watchers(),
    [
        'joe_engineer@sanger.ac.uk', 'full@address.already',
        'joe_results@sanger.ac.uk'
    ],
...
cmp_deeply(
    $test[0]->batch_details(),
    {
        batch_id => 4861,
        error    => q{},
        lanes    => $lane_detail
    },
    'Return correct batch details'
);
...
like(
    $bd->{error},
qr/Attribute \(batch_id\) does not pass the type constraint because: Validation failed for 'NpgTrackingPositiveInt' (failed\s)?with value 0/,
    'Error string for batch id zero is correct'
);

lives_ok {
    $test[2] = npg::email::event::status_change::run->new(
        {
            id_event          => 26,
            schema_connection => $schema,
        }
...
like(
    $test[2]->batch_details()->{error},
qr/Attribute \(position\) does not pass the type constraint because: Validation failed for 'NpgTrackingLaneNumber' (failed\s)?with value 10/,
    'Error string for batch with too many lanes'
);
...
like( $test[0]->email_body_store()->[0],
    qr{^\Q$header\E}ms, 'Non-live emails have a special header' );
...
# Be careful not to test send in this block.
{
    ...
    unlike( $test[3]->email_body_store()->[0],
        qr{^$header}ms, 'Live emails have no special header' );
}

like(
    $test[0]->email_body_store()->[0],
    qr{(?:Lane - \d+: Library - .+\n){8}}ms,
    'The lane template has been processed'
);

like( $test[0]->email_body_store()->[0],
    qr{$lane_detail_re}ms, 'The relevant lane details have been included' );

like( $test[0]->email_body_store()->[0],
    qr{$npg_run_url}ms, 'NPG run page address has been included' );

like( $test[0]->email_body_store()->[0],
    qr{$signature$}ms, 'Signature has been included' );

my $reports;
lives_ok { $reports = $test[0]->compose_st_reports() }
'compose_st_reports survives';
...

{
    ...
    is(
        $watchers->[0],
        $ENV{USER} . $test[4]->default_recipient_host(),
