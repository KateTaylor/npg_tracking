...

{
    ...
    is(
        $ev->live(),
        'http://psd-support.internal.sanger.ac.uk:6600/events',
        'live url returned'
    );
    is(
        $ev->dev(),
        'http://psd-dev.internal.sanger.ac.uk:6800/events',
        'dev url returned'
    );
    is( $ev->fields(), 'key', 'last of fields is key' );

    my $XML =
q[<?xml version='1.0'?><event><message>Run 10 : %s</message><eventful_id>11</eventful_id><eventful_type>Item</eventful_type><family>%s</family><identifier>10</identifier><key>%s</key><location>4</location></event>];
    ...
    for my $state ( keys %{$STATES} ) {
        ...
        is(
            $ev->create(
                { run_status => $state, id_run => 10, position => 4, }
            ),
            1,
            "$state => $family"
        );
...

{
    my $ev = st::api::event->new(
        {
            util => npg::api::util->new(
                { useragent => t::useragent->new( { is_success => 1, } ), }
            ),
            message       => 'a message',
            eventful_type => 'run',
            eventful_id   => 999,
    ...
    is(
        $req->{Content},
q(<?xml version='1.0'?><event><message>a message</message><eventful_type>run</eventful_type><eventful_id>999</eventful_id><family>update</family><identifier>100</identifier><location>1</location></event>),
        'xml generation ok'
    );
}

{
    my $ev = st::api::event->new(
        {
            util => npg::api::util->new(
                { useragent => t::useragent->new( { is_success => 0, } ), }
            ),
            service       => 'dev',
            message       => 'a message',
            eventful_type => 'run',
