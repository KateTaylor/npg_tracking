...
);
{
    ...
    is( $view->convert_is_good(1), 'Good',
        '$view->convert_is_good(1) is Good' );
    is( $view->convert_is_good(2),
        'Unknown', '$view->convert_is_good(2) is Unknown' );
    is( $view->convert_is_good(3), 'Bad', '$view->convert_is_good(3) is Bad' );

    is( $view->selected_days(), 14,
        '$view->selected_days() gives default 14 days if not set as cgi param'
    );
    $cgi = $view->util->cgi();
    $cgi->param( 'days', 7 );
    is( $view->selected_days(),
        7, '$view->selected_days() gives selected days if set as cgi param' );
}
