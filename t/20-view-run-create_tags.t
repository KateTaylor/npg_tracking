...

{
    ...
    like(
        ...
        'not authorised to create tags if not admin'
    );
}

{
    ...
    ok(
        $util->test_rendered(
            $str, 't/data/rendered/run/1.html;update_tags-no-tags'
        ),
        'list render is ok - no tags added'
    );
}

{
    ...
    ok(
        $util->test_rendered( $str, 't/data/rendered/run/1.html;update_tags' ),
        'list render is ok - tags added'
    );
...

{
    ...
    ok(
        $util->test_rendered( $str, 't/data/rendered/run/1.html;update_tags' ),
        'list render is ok - tags removed'
    );
    my $run = npg::model::run->new(
        {
            util   => $util,
