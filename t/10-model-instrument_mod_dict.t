...
use_ok('npg::model::instrument_mod_dict');
{
    ...
    is(
        $imds->[0]->description(),
        'Mode Scrambler',
        'first row object description is Mode Scrambler'
    );
    ...
    is(
        $descriptions->[0]->[0],
        'Mode Scrambler',
        'first description is Mode Scrambler'
    );
    ...
    is( $model->id_instrument_mod_dict(),
        12, 'id_instrument_mod_dict still the same' );
}
