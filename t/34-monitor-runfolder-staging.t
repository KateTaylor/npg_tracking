...

{
    ...
    'Object creation ok';
    is( $test->netcopy_complete_wait,
        600, 'default netcopy complete wait time' );
}

{
    ...
    is(
        $test->_get_folder_path_glob,
        $MOCK_STAGING . '/IL3/*/',
        'internal glob correct'
    );
    lives_ok { $test->update_folder } 'update folder name and glob in DB';
    is( $test->run_db_row->folder_name(),
        '100622_IL3_01234', '  folder name updated' );
    is(
        $test->run_db_row->folder_path_glob(),
        $MOCK_STAGING . '/IL3/*/',
...

{
    ...
    is( $test->validate_run_complete(),
        0, 'Called validate_run_complete before Events.log is complete' );
    ...
    is( $test->validate_run_complete(), 1, '\'run complete\' status is valid' );
}
...

{
    ...
    isnt( $test->current_run_status_description(),
        'run mirrored', '\'run mirrored\' status not set' );
    ...
    is( $test->current_run_status_description(),
        'run mirrored', '   ...and the \'run mirrored\' status is set' );
    ...
    ok(
        $first_flag_mtime < $second_flag_mtime,
    ...
    my @missing_cycles;
    lives_ok { @missing_cycles = $test->missing_cycles($mock_path); }
    q{missing_cycles ran ok};
...

{
    ...
    isnt(
        $test->current_run_status_description(),
        'analysis pending',
...
{
    my $tmpdir = tempdir( CLEANUP => 1 );
    system( 'cp', '-rp', $MOCK_STAGING . '/IL5/incoming/100621_IL5_01204',
        $tmpdir );
...
{
    my $test = Monitor::RunFolder::Staging->new( runfolder_path =>
          '/nfs/sf25/ILorHSany_sf25/outgoing/110712_HS8_06541_B_B0A6DABXX' );
    is(
        $test->_get_folder_path_glob,
        '/{export,nfs}/sf25/ILorHSany_sf25/*/',
        'glob for /nfs/sf25 outgoing'
    );
    $test = Monitor::RunFolder::Staging->new( runfolder_path =>
          '/export/sf25/ILorHSany_sf25/incoming/110712_HS8_06541_B_B0A6DABXX' );
    is(
        $test->_get_folder_path_glob,
        '/{export,nfs}/sf25/ILorHSany_sf25/*/',
        'glob for /export/sf25 incoming'
    );
}
