...

{
    ...
    isa_ok( $current_instrument_statuses, 'ARRAY',
        '$model->current_instrument_statuses()' );
    is( $model->current_instrument_statuses(),
        $current_instrument_statuses,
        '$model->current_instrument_statuses() cached ok' );
    $model->{'current_instrument_statuses'} = undef;
    my $limit_current_instrument_statuses =
      $model->current_instrument_statuses(1);
    isnt( $limit_current_instrument_statuses,
        $current_instrument_statuses,
        'retrieves with a limit included - different result to without limit' );
    my $latest_current_instrument_status =
      $model->latest_current_instrument_status();
    isa_ok(
        $latest_current_instrument_status,
    ...
    is(
        $model->latest_current_instrument_status(),
        $latest_current_instrument_status,
        '$model->latest_current_instrument_status() cached ok'
    );

    #######
    # test getting the objects related to join table
    is( $already_at_status_up->instrument->name(),
        'IL10',
        '$model->instrument() returns correct object - checked via name' );
    is( $already_at_status_up->user->username(),
        'joe_admin',
        '$model->user() returns correct object - checked via username' );
    is( $already_at_status_up->instrument_status_dict->description(), 'up',
'$model->instrument_status_dict() returns correct object - checked via description'
    );
    ...
    throws_ok { $model->_check_order_ok(); } qr//,
      q{croaked on moving status to 'request approval' from 'up'};
    ...
    lives_ok { $model->_check_order_ok(); }
    q{no croak on moving status to 'wash required' from 'up'};
    ...
    lives_ok { $model->_check_order_ok(); }
    q{no croak on moving status to 'planned repear' from 'up'};

    $model = npg::model::instrument_status->new(
        {
            util => $util,
            id_instrument =>
              $already_at_status_wash_required->instrument->id_instrument(),
    ...
    $model = npg::model::instrument_status->new(
        {
            util => $util,
            id_instrument =>
              $already_at_status_down->instrument->id_instrument(),
...
qr/Instrument IL4 \"wash required\" status cannot follow current \"down\" status/,
    ...
    lives_ok { $model->_request_approval(); }
    q{no croak on request approval for moving status to not 'up'};
    lives_ok { $model->_check_order_ok(); }
    q{no croak on moving status to 'request approval' from 'down'};

    $model = npg::model::instrument_status->new(
        {
            util => $util,
            id_instrument =>
              $already_at_status_request_approval->instrument->id_instrument(),
            id_user => $already_at_status_request_approval->user->id_user(),
            id_instrument_status_dict => 1,
        }
    );
    lives_ok { $model->_check_order_ok(); }
    q{no croak on moving status to 'up' from 'request approval'};
    ...
    lives_ok { $model->_request_approval(); }
q{no croak on request approval for moving status to 'up' as user is an approver};
}

{
    ...
    q{no croak on create of 'down for repair' status for id_instrument 13};
    $model =
      npg::model::instrument->new( { util => $util, id_instrument => 13 } );
    cmp_ok(
        npg::model::instrument->new( { util => $util, id_instrument => 13 } )
          ->current_instrument_status->instrument_status_dict->description,
        'eq',
        'down for repair',
        'down for repair is current status in database'
    );
}

{
    ...
    $util->catch_email($model);
    lives_ok { $model->create(); }
    'no croak on create of wash in progress status for id_instrument 8';
    ...
    $util->catch_email($model);
    lives_ok { $model->create(); }
    'no croak on create of wash performed status for id_instrument 8';

    cmp_ok(
        npg::model::instrument->new( { util => $util, id_instrument => 8 } )
          ->current_instrument_status->instrument_status_dict->description,
        'eq',
        'up',
        'up is current status in database (automatic from wash performed)'
    );
    ...
    $util->catch_email($model);
    lives_ok { $model->create(); }
    'no croak on create of planned repair status for id_instrument 8';
}
...

{
    ...
    isa_ok( $model->utilisation('hour'),
        'ARRAY', 'run by hour, via call in npg::model::instrument' );
    is( scalar @{ $model->utilisation('hour') },
        720, 'run by hour, via call in npg::model::instrument' );
}
...

{
    ...
    is( scalar @{ $model->annotations },
        2, 'two annotations returns for this instrument_status' );
    is(
        $model->annotations->[1]->comment,
        'Solexa test flowcell - New paired end module',
        'correct comment for the second annotation'
    );
}
