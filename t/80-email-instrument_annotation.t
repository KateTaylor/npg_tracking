...
my $test;
lives_ok {
    $test = npg::email::event::annotation::instrument->new(
        { event_row => $event_row, schema_connection => $schema, } );
}
q{Can create with event row object};

is( $test->template(), q{instrument_annotation.tt2},
    q{correct template name obtained} );
is( $test->user(), q{pipeline}, q{user returns username} );
is(
    $test->event_row->description(),
q{pipeline annotated instrument HS8 - Monitor lane 8 in subsequent runs for right edge of tiles 51-100 out of focus.},
    q{annotation retrieved from description}
);
...
is_deeply(
    $watchers,
    [qw{joe_engineer@sanger.ac.uk}],
    q{watchers for engineers is correct}
);
...
is(
    $email->{subject},
    q{Instrument HS8 has been annotated by pipeline} . qq{\n},
